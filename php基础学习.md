...
##### 1、`TCP` 三次握手流程
```
三次握手  猥琐男对卷发妹说：一起吃饭吧，卷发妹回应：好啊，猥琐男收到回应

四次握手  猥琐男对卷发妹说：一起吃饭吧，卷发妹回应：好啊，等下，我还有点事忙完就去，卷发妹对猥琐男说：忙完了，可以出发了，猥琐男知晓后回应：好，我们出发
```
##### 2、`TCP`、`UDP` 区别，分别适用场景
...
```
APP_ID : 用户id，APP_SECRET：用户密钥，这是平台分配给用户的基本信息，只有这些信息验证通过了才允许使用此api
```
##### 4、`API` 请求如何保证数据不被篡改？
```
篡改api就是通过抓包获取到请求接口的接口参数，通过修改相关参数，达到欺骗服务器的目的，常用的防篡改api方式是通过加密和签名
```
##### 5、`JSON` 和 `JSONP` 的区别
```
...
##### 7、`RSA` 是什么
```
一种非对称加密算法，将公钥公开出去和明文做加密，私钥可以将加密后的明文解密出来
```
##### 8、`API` 版本兼容怎么处理
##### 9、限流（木桶、令牌桶）
```
木桶算法限流：包子铺每天正常情况下，来一个客人给钱就把包子卖出去，现在生意好了，突然来了十个人一起买，为了调整秩序，就让这些人排队给钱领包子

令牌桶限流：包子铺发现排队的人太多了，包子可能不够卖，导致可能给了钱领不到包子，为了解决这一难题，就在提前准备包子对应的小票，给钱时给客人一张小票，最后没有小票的人就不用给钱领包子了
```
##### 10、`OAuth2` 主要用在哪些场景下
```
...
c.密码模式（resource owner password credentials）我们用的这种
