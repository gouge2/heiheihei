    ...
    <script type="text/javascript">
        ...
        function changestatus(id, status) {
            if (id != '') {
                $.ajax({
                    type: "POST",
                    url: "/taokeyun.php/GoodsCat/changestatus",
                    dataType: "html",
        ...
        function changetop(id, status) {
            if (id != '') {
                $.ajax({
                    type: "POST",
                    url: '/taokeyun.php/GoodsCat/changetop',
                    dataType: "html",
                    data: "id=" + id + "&status=" + status,
                    success: function (msg) {
                        if (msg == '1') {
        ...
        <div class="layui-col-md12">
            <div class="layui-card">
                ...
                <div class="ibox-content">
                        <a class="layui-btn pull-left " href="/merchant/app/index.php?i=1&c=entry&m=shop&do=mobile&r=merch.checkCat">同步商户端分类</a>
                        <a class="layui-btn pull-left" href="__MODULE__/Goods/examineGoods">待审核商品</a>
                    <div class="row">
                        <a class="layui-btn pull-right" href="__CONTROLLER__/add">添加商品分类</a>
                    </div>
                    <div class="layui-row layui-col-space15">
                            ...
                            <table class="layui-table">
                                ...
                                <foreach name="catlist" item="v">
                                    ...
                                    <tr class="{$tr_class}">
                                        ...
                                        <td>
                                            <if condition='$v.is_show eq Y'>
                                                <button type="button" class="layui-btn layui-btn-xs"
                                                        onclick="changestatus({$v.cat_id},'N');">显示
                                                </button>
                                                <else/>
                                                <button type="button" class="layui-btn layui-btn-primary layui-btn-xs"
                                                        onclick="changestatus({$v.cat_id},'Y');">隐藏
                                                </button>
                                            </if>
                                        </td>
                                        <td>
                                            <if condition='$v.is_top eq Y'>
                                                <button type="button" class="layui-btn layui-btn-xs"
                                                        onclick="changetop({$v.cat_id},'N');">&nbsp;&nbsp;置顶&nbsp;&nbsp;&nbsp;
                                                </button>
                                                <else/>
                                                <button type="button" class="layui-btn layui-btn-primary layui-btn-xs"
                                                        onclick="changetop({$v.cat_id},'Y');">&nbsp;不置顶&nbsp;
                                                </button>
                                            </if>
                                        </td>
                                        <td>{$v.create_time}</td>
                                        <td>
                                            <a href="__MODULE__/Goods/index/cat_id/{$v.cat_id}" class="layui-btn">查看商品列表（<?php echo $goods_num; ?>
                                                件）</a></td>
                                        <td class="has-warning"><input name="sort[{$v.cat_id}]" value="{$v.sort}"
                                                                       class="form-control"
                                        ...
                                        ?>
                                        <td><a href="__MODULE__/GoodsAttribute/index/goods_cat_id/{$v.cat_id}" class="layui-btn">商品分类属性配置（{$attribute_num}类）</a>
                                        </td>
                                        <td>
