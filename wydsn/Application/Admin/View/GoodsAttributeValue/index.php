...
function changeshow(id,status)
{
	if(id!='') {
		$.ajax({
			type:"POST",
			url:'/taokeyun.php/GoodsAttributeValue/changeshow',
			dataType:"html",
...
function del(goods_attribute_id,goods_attribute_value_id,value)
{
	if(goods_attribute_id!='' && goods_attribute_value_id!='' && value!='')
	{
		swal({
			...
			$.ajax({
				type:"POST",
				url:'/taokeyun.php/GoodsAttributeValue/del',
				dataType:"html",
        ...
        <div class="layui-col-md12">
            <div class="layui-card">
                <div class="ibox-content">
						<h3>当前位置： 商城系统 &raquo; 商品分类属性管理 &raquo; 商品分类属性值管理<a class="layui-btn pull-right" href="__MODULE__/GoodsAttribute/index/goods_cat_id/{$GoodsAttributeMsg.goods_cat_id}" style="margin-top: -10px">返回商品分类属性配置 <i class="fa fa-angle-double-right"></i></a></h3>
					</div>
					<div class="ibox-content">
						<div class="row">
							<a class="layui-btn pull-right" href="__CONTROLLER__/add/goods_attribute_id/{$GoodsAttributeMsg.goods_attribute_id}">添加商品分类属性值</a>
       					</div>
						<div class="layui-row layui-col-space15">
							...
							<table class="layui-table">
								...
								<tbody>
									<foreach name="list" item="l">
                                       <tr>
                                          ...
                                          <td>
                                            <if condition='$l[is_show] eq Y'>
                                               <button type="button" class="layui-btn btn-primary btn-sm" onclick="changeshow({$l.goods_attribute_value_id},'N');">显示</button>
                                			<else/>
                                			   <button type="button" class="layui-btn btn-danger btn-sm" onclick="changeshow({$l.goods_attribute_value_id},'Y');">不显示</button>
                                			</if>
                                		  </td>
									  ...
									  <tr>
										<td colspan="6">
	        								<input type="submit" class="layui-btn btn-primary pull-right" value="统一排序">
		  								</td>
	   								 </tr>
								</tbody>
							</table>
