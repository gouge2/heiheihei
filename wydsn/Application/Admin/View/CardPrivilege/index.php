    ...
    <script type="text/javascript">
            ...
            $('#transfer').click(function(){
                ...
                if(all_id!='') {
                    ...
                    swal({
                        ...
                        $.ajax({
                            type:"POST",
                            url:"/taokeyun.php/CardPrivilege/transfer",
                            dataType:"html",
        ...
        function changeshow(id,status)
        {
            if(id!='')
            {
                $.ajax({
                    type:"POST",
                    url:'/taokeyun.php/CardPrivilege/changeshow',
                    dataType:"html",
        ...
        function changetop(id,status)
        {
            if(id!='')
            {
                $.ajax({
                    type:"POST",
                    url:'/taokeyun.php/CardPrivilege/changetop',
                    dataType:"html",
                    data:"id="+id+"&status="+status,
                    success:function(msg)
                    {
        ...
        function changesale(id,status)
        {
            if(id!='')
            {
                $.ajax({
                    type:"POST",
                    url:'/taokeyun.php/CardPrivilege/changesale',
                    dataType:"html",
                    data:"id="+id+"&status="+status,
                    success:function(msg)
                    {
        ...
        function del(id)
        {
            if(id!='') {
                swal({
                    ...
                    $.ajax({
                        type:"POST",
                        url:'/taokeyun.php/CardPrivilege/del',
                        dataType:"html",
                        data:"id="+id,
                        success:function(msg)
                        {
        ...
        <div class="layui-col-md12">
            <div class="layui-card">
                <div class="ibox-title">
                    <h3>当前位置： 黑卡管理系统 &raquo; {$cname}</h3>
                </div>
                <div class="ibox-content">
                    <form action="__ACTION__" method="get" role="form" class=" pull-left layui-form" >
                        <!-- 分类
                        ...
                        <div class="layui-form-item">
                            ...
                            <div class="layui-inline">
                                ...
                                <div class="layui-input-inline">
                                    <input type="text" name="search" required  autocomplete="off" class="layui-input">
                                </div>
                            </div>
                            <div class="layui-inline">
...
<!--                        <button class="btn btn-primary" type="submit">查询</button>-->
                    ...
                    <a class="layui-btn pull-right" href="__CONTROLLER__/add">添加商品</a>
                    <div class="layui-row layui-col-space15">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <if condition="$g.tequan eq 'Y'">
                                                <button type="button" class="layui-btn layui-btn-xs" onclick="changetop({$g.id},'N');">特权</button>
                                                <else/>
                                                <button type="button" class="layui-btn layui-btn-danger layui-btn-xs" onclick="changetop({$g.id},'Y');">不特权</button>
                                            </if>
                                        </td>
                                        <td>
                                            <if condition='$g.status eq Y'>
                                                <button type="button" class="layui-btn layui-btn-xs" onclick="changesale({$g.id},'N');">是</button>
                                                <else/>
                                                <button type="button" class="layui-btn layui-btn-danger layui-btn-xs" onclick="changesale({$g.id},'Y');">否</button>
                                            </if>
                                        </td>
