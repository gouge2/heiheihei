...
class TbGoodsController extends AuthController
{
    	...
    	if(I('post.'))
    	{
    		...
    		if(trim(I('post.goods_id')) and trim(I('post.goods_name')))
    		{
    			...
    			if($gy_data['couponInfo'])
    			{
    				...
    				$coupon_amount=substr($gy_data['couponInfo'], $pos1+3,$pos2-$pos1-3);
    			}else {
    				$coupon_amount=0;
    			}
    			//保存到数据库
    			$data=array(
    			    ...
    			    'commission_rate'=>$gy_data['maxCommissionRate'],
    			    'volume'=>$volume,
    ...
    public function set()
    {
        $where="agent_id=0";
        $TbGoods=new \Common\Model\TbGoodsModel();
        $list = $TbGoods->where($where)->order('sort desc,id desc')->select();
        if($list) {
            Vendor('tbk.tbk','','.class.php');
            $tbk=new \tbk();
            foreach ($list as $l){
                ...
                if(!$TbGoods->create($data))
                {
                    ...
                    // 验证成功
                    $TbGoods->where("id='$id'")->save($data);
                    continue;
                }
            }
            $this->success('更新淘宝商品成功！');
        }else {
            $this->error('没有淘宝商品可更新！');
        }
    }
}
?>