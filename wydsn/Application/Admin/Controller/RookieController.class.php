    ...
    public function add()
    {
        ...
        if (I('POST.')) {
            ...
            */
            ...
            // 组装数据
            if($exs_time<=$end_time)
            {
                $this->error('兑换开始时间需要大于活动结束时间');
            }
            if($exs_time >=$exe_time)
            {
                $this->error('兑换结束时间需要大于兑换开始时间');
            }
            $data = array(
                'name'          => I('POST.name'),
...
//            $exe_time = $exe_time.':00';
            ...
            $data['exs_time'] = date('Y-m-d',strtotime($data['exs_time']));
            $data['exe_time'] = date('Y-m-d',strtotime($data['exe_time']));
            // 模板渲染
            $this->assign('data', $data);
            $this->assign('id', $id);
