    ...
    public function add()
    {
        if($_POST){
            ...
            if(trim(I('post.name'))){
                ...
                $DiyModule=new \Common\Model\DiyModuleModel();
                //手动添加的ID
                if (trim(I('post.new_id'))){
                    $new_id = trim(I('post.new_id'));
                    $list=$DiyModule->getModuleList();
                    $ids=array_column($list, 'id');
                    if (in_array($new_id,$ids)){
                        $this->error('功能模块ID已存在，请重新添加！');
                    }
                    $data['id']=$new_id;
                }
                $DiyModule=new \Common\Model\DiyModuleModel();
    ...
    public function edit($id)
    {
        ...
        if($_POST){
            ...
            if(trim(I('post.name'))){
                //修改ID
                if (trim(I('post.new_id'))){
                    if (trim(I('post.new_id')) != trim(I('post.old_id'))){
                        $new_id = trim(I('post.new_id'));
                        $list=$DiyModule->getModuleList();
                        $ids=array_column($list, 'id');
                        if (in_array($new_id,$ids)){
                            $this->error('功能模块ID已存在，请重新修改！');
                        }
                    }
                }else{
                    $this->error('功能模块ID不能为空！');
                }
