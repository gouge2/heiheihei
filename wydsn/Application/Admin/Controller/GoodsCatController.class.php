    	...
    	if(I('post.'))
    	{
    		...
    		if(!$GoodsCat->create($data))
    		{
    			...
    			if($res)
    			{
                    ...
                    {
                        $lvl = $GoodsCat->getCatLevel($data['parent_id']);
                        $data = array_merge($data,['level'=>$lvl+1]);
                    }else{
                        $data = array_merge($data,['level'=>1]);
                    }
                    $data = array_merge($data,['id'=>$GoodsCat->getLastInsID()]);
                    $GoodsCat->syncCategoryToMerch(0,$data,'add');
    	...
    	if(I('post.'))
    	{
    		...
    		if(!$GoodsCat->create($data))
    		{
    			...
    			if($res!==false)
    			{
    				...
    				$data = array_merge($data,['id'=>$cat_id]);

                    $GoodsCat->syncCategoryToMerch($cat_id,$data,'update');
    	...
    	*  */
    	...
    	if($goods_num>0)
    	{
    		...
    		if($res!==false)
    		{
    			...
    			$GoodsCat->syncCategoryToMerch($cat_id,[],'del');
    			echo '1';
    		}else {
    			echo '0';
    		}
