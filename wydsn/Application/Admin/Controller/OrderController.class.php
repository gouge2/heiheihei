...
class OrderController extends AuthController
{
    ...
    public function unpaid()
    {
        ...
        foreach (self::$short_base['status'] as $key => $val) {
            if (I('get.type') == $key) {
                self::$short_base['status'][$key]['sel'] = 1;
            }
        }
        // 分页显示输出
        ...
        $this->assign('search', self::$short_base);
        $this->display();
    }
    ...
    public function orderSetting()
    {
        ...
        $msg['receive'] = defined('RECEIVE')?RECEIVE:'';//自动确认收货
        $msg['closeorder'] = defined('CLOSEORDER')?CLOSEORDER:'';//关闭未付款订单
        $this->assign('msg',$msg);
        $this->display();
    }
}
