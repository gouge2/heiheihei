...
class DbBackupController extends AuthController
{
	...
	{
		layout(false);
		Vendor ('DbBackup/DbBackup','','.class.php');
		if (IS_POST && ! empty ( $tables ) && is_array ( $tables )) 
		{
			...
			// 创建备份文件
			$DbBackup = new \DbBackup( $file, $config );
			if (false !== $DbBackup->create ()) 
			{
				$tab = array (
						'id' => 0,
						'start' => 0 
			...
			// 备份指定表
			$DbBackup = new \DbBackup ( session ( 'backup_file' ), session ( 'backup_config' ) );
			$start = $DbBackup->backup ( $tables [$id], $start );
			if (false === $start) 
			{
				// 出错
				$this->error ( '备份出错！' );
	...
	public function import($time = 0, $part = null, $start = null)
	{
		...
		if (is_numeric ( $time ) && is_null ( $part ) && is_null ( $start )) 
		{
			...
			$db = new \DbBackup ( $list [$part], array (
