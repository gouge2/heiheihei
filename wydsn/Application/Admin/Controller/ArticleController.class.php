    ...
    public function add($cat_id)
    {
    	...
    	if(I('post.'))
    	{
    		...
    		if(!empty($content))
    		{
    			...
    			//视频文件图片全部替换
                $content=str_replace('src="/Public/Upload/ueditor/','src="'.WEB_URL.'/Public/Upload/ueditor/',$content);
    		}
    		//上传标题图片
    		if(!empty($_FILES['img']['name']))
    		{
                ...
                $img='/Public/static/admin/img/logo.png';
            }
    		//上传大图片
            ...
            if(!empty($_FILES['video']['name'])) {
                $config = array(
                    'mimes'         =>  array(), //允许上传的文件MiMe类型
                    'maxSize'       =>  1024*1024*50, //上传的文件大小限制 (0-不做限制)
                ...
                $upload = new \Think\Upload($config);
                // 上传单个文件
                $info = $upload->uploadOne($_FILES['video'],1);
                if(!$info) {
                    // 上传错误提示错误信息
                    $this->error($upload->getError());
                }else{
                    ...
                    $video=ltrim($filepath3,'.');
                    $video = 'http://'.$_SERVER['HTTP_HOST'].$video;
                }
            }
//    		//上传文件
    	...
    	if(I('post.'))
    	{
            ...
            if(!empty($_FILES['video']['name'])) {
                $config = array(
                    ...
                    'saveExt'       =>  '', //文件保存后缀，空则使用原后缀
                    'replace'       =>  true, //存在同名是否覆盖
                    'saveName'      =>  'article_video'.time(), //上传文件命名规则，[0]-函数名，[1]-参数，多个参数使用数组
                );
                $upload = new \Think\Upload($config);
                // 上传单个文件
                $info = $upload->uploadOne($_FILES['video'],1);
...
//    				'file'=>$file,
    		...
    		if(!$Article->create($data))
    		{
    			...
    			if($res!==false)
    			{
    				...
    				if(I('post.oldimg') and $img!=I('post.oldimg') and I('post.oldimg')!='/Public/static/admin/img/logo.png')
    				{
    				...
    				if(I('post.oldbigimg') and $bigimg!=I('post.oldbigimg'))
    				{
						...
						if ($oldbigimg != '/Public/static/admin/img/logo.png') {
							unlink('.'. $oldbigimg);
						}
    				}
                    // 如果原视频是上传的，又上传新视频则删掉旧视频
                    if(!empty($_FILES['video']['name']) and trim(I('post.oldhref')) and strpos(trim(I('post.oldhref')),$_SERVER['HTTP_HOST']))
                    {
						$oldhref='/'.ltrim(trim(I('post.oldhref')),'http://'.$_SERVER['HTTP_HOST']);
						if ($oldhref != '/Public/static/admin/img/logo.png') {
							unlink('.'. $oldhref);
						}
                    }
//    				// 删除原文件
...
//    				@unlink($filepath3);
    ...
    public function deloldimg($article_id)
    {
    	...
    	if($res===false)
    	{
    		...
    		if($res2)
    		{
				if ($oldimg !=  '/Public/static/admin/img/logo.png') {
					$oldimg='.'.$oldimg;
					unlink($oldimg);
				}
    			echo '1';
    ...
    public function deloldfile($article_id)
    {
    	...
    	if($res===false)
    	{
    		...
    		if($res2)
    		{
				if ($oldfile != '/Public/static/admin/img/logo.png') {
					unlink('.'. $oldfile);
				}
    			echo '1';
    		}else {
