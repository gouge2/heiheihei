    ...
    public function changeImagePath($content)
    {
        ...
        if (!empty($img)) {
            ...
            foreach ($img as $k => $v) {
                ...
                if ((strpos($v, $img_url) === false) && (strpos($v, 'http://') === false) && (strpos($v, 'https://') === false)) {
                    $final_imgUrl = $img_url . $v;
                } else {
                    $final_imgUrl = $v;
                }
                $final_imgUrl = str_replace("//Public", "/Public", $final_imgUrl);
                $replacements[] = $final_imgUrl;
                $img_new = "/" . preg_replace("/\//i", "\/", $v) . "/";
                $patterns[] = $img_new;
            }
            //var_dump($patterns,$replacements);exit;
            //让数组按照key来排序
            ksort($patterns);
            ksort($replacements);
            //替换内容
