...
class VipCatModel extends Model
{
    //验证规则
    protected $_validate =array(
        ...
        array('vip_id','require','唯品会官方分类ID不能为空！',self::EXISTS_VALIDATE),  //存在验证，必填
        array('vip_id','is_natural_num','唯品会官方分类ID不存在！',self::EXISTS_VALIDATE,'function'),  //存在验证，必须是自然数
    );
    ...
    public function getGoodsCatList($is_show='')
    {
        ...
        $cat=$this->where($where)->order('sort desc,vip_cat_id asc')->select();
        if($cat!==false)
        {
            $list =$this->rule($cat);
            return $list;
    ...
    public function getParentList($is_show='')
    {
        ...
        $list=$this->where($where)->order('sort desc,vip_cat_id asc')->select();
        if($list!==false)
        {
            return $list;
        }else {
    ...
    public function getSubListByParent($pid,$order='asc',$is_show='')
    {
        ...
        $list=$this->where($where)->order("sort desc,vip_cat_id $order")->select();
        if($list!==false)
        {
            return $list;
        }else {
    ...
    public function getSubList($taobao_cat_id,$order='asc',$is_show='')
    {
        ...
        $cat=$this->where($where)->order("sort desc,vip_cat_id $order")->select();
    ...
    /**
     * 获取商品分类信息
     * @param int $vip_cat_id:商品分类ID
     * @return array|boolean
     */
    public function getMsg($vip_cat_id)
    {
        $msg=$this->where("vip_cat_id='$vip_cat_id'")->find();
        if($msg!==false)
        {
            return $msg;
        }else {
    ...
    static public function rule($cate , $lefthtml = '— ' , $pid=0 , $lvl=0, $leftpin=0 )
    {
        ...
        foreach ($cate as $v)
        {
            if($v['pid']==$pid)
            {
                ...
                $arr[]=$v;
                $arr= array_merge($arr,self::rule($cate,$lefthtml,$v['vip_cat_id'],$lvl+1 , $leftpin+20));
            }
        }
        return $arr;
    }
