...
class BannerModel extends Model
{
	 ...
	 */
	public function getBannerList($cat_id, $is_show = '', $agent_id = 0, $field = '*', $ad = true)
	{
		// 判断分类是否显示
        if ($ad) {
            $BannerCat = new \Common\Model\BannerCatModel();
            $cat = $BannerCat->getCatMsg($cat_id);
        ...
        $whe = ['cat_id' => $cat_id, 'agent_id' => $agent_id];

        if ($is_show) {
            $whe['is_show'] = $is_show;
        }

        $res = $this->field($field)->where($whe)->order('sort desc')->select();

        if ($res !== false) {
            foreach ($res as $k => $banner) {
                list($width, $height) = getimagesize(getUrls() . $banner['img']);
                $res[$k]['width'] = $width;
                $res[$k]['height'] = $height;
                if ($res[$k]['img']) {
                    $res[$k]['img'] = (is_url($banner['img']) ? $banner['img'] : getUrls() . $banner['img']);
                }
            }
