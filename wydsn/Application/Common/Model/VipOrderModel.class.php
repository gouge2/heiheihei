...
class VipOrderModel extends Model
{
    ...
    {
        $msg=$this->where("orderSn='$order_sn'")->find();
        if($msg)
        {
            //给购买会员返利
            $uid=$msg['user_id'];
            ...
            if($groupMsg and $UserMsg)
            {
                ...
                if(!$User->create($data_user))
                {
                    ...
                    $res_record=$UserBalanceRecord->addLog($uid, $money_user, $all_money, 'vip','2',$order_sn,'4');
                    ...
                    $res_order=$this->where("orderSn='$order_sn'")->save($data_order);
                    ...
                    if($res_balance!==false and $res_record!==false and $res_order!==false)
                    {
                        ...
                        if($UserMsg['referrer_id'] and ($UserMsg['group_id']=='1' or $UserMsg['group_id']=='2'))
                        {
                            ...
                            $res_record_r=$UserBalanceRecord->addLog($referrer_id, $referrer_money, $all_money_r, 'vip_r','2',$order_sn,'4');
                            ...
                            if($res_balance_r!==false and $res_record_r!==false) {
                                ...
                                if($referrerMsg['referrer_id'])
                                {
                                    ...
                                    if($referrer_money2>0)
                                    {
                                        ...
                                        $res_record_r2=$UserBalanceRecord->addLog($referrer_id2, $referrer_money2, $all_money_r2, 'vip_r2','2',$order_sn,'4');
                                    }else {
                                        $res_balance_r2=true;
                                        $res_record_r2=true;
                                    }
                                                ...
                                                if($is_vip<2)
                                                {
                                                    ...
                                                    if($referrer_tgid=='3' or $referrer_tgid=='4')
                                                    {
                                                        ...
                                                        if($referrer_money_team>0)
                                                        {
                                                            ...
                                                            $res_record_rt=$UserBalanceRecord->addLog($referrer_tid, $referrer_money_team, $all_money_rt, 'vip_rt','2',$order_sn,'4');
                                                            //保存团队分红订单记录
//                                                            $TeamRewardsLog=new \Common\Model\TeamRewardsLogModel();
//                                                            $res_reward_rt=$TeamRewardsLog->addLog($referrer_tid, $uid, $order_sn, 'vip',$is_vip+1);
        ...
        if($msg)
        {
            ...
            if($res_exist)
            {
                ...
                if($groupMsg and $UserMsg)
                {
                    ...
                    //保存余额变动记录-预估
                    $create_time=date('Y-m-d H:i:s',$msg['ordertime']/1000);
                    $res_record=$UserBalanceRecordTmp->addLog($uid,$money_user,$action='vip',$order_sn,$type='4',$create_time);
                    if($res_record!==false)
                    {
                        //给直接推荐、间接推荐人返利
                        //并且购买用户是普通会员
                        if($UserMsg['referrer_id'] and ($UserMsg['group_id']=='1' or $UserMsg['group_id']=='2'))
                        {
                            ...
                            $referrer_money=round($referrer_money,2);
                            //保存余额变动记录
                            $res_record_r=$UserBalanceRecordTmp->addLog($referrer_id,$referrer_money,$action='vip_r',$order_sn,$type='4',$create_time);
                            if($res_record_r!==false)
                            {
                                ...
                                if($referrerMsg['referrer_id'])
                                {
                                    ...
                                    if($referrer_money2>0)
                                    {
                                        ...
                                        $referrer_money2=round($referrer_money2,2);
                                        //保存余额变动记录
                                        $res_record_r2=$UserBalanceRecordTmp->addLog($referrer_id2,$referrer_money2,$action='vip_r2',$order_sn,$type='4',$create_time);
                                    }else {
                                        $res_record_r2=true;
                                    }
...
//															$referrer_money_team=$money*0.05;
                                                        ...
                                                        {
                                                            //保存余额变动记录
                                                            $res_record_rt=$UserBalanceRecordTmp->addLog($referrer_tid,$referrer_money_team,$action='vip_rt',$order_sn,$type='4',$create_time);
                                                            //保存团队分红订单记录
                                                            $TeamRewardsLog=new \Common\Model\TeamRewardsLogModel();
                                                            $res_reward_rt=$TeamRewardsLog->addLog($referrer_tid, $uid, $order_sn, 'vip',$is_vip+1);
