...
class OrderDetailModel extends Model
{
	...
	{
        $sql="SELECT o.*,g.img,g.clicknum,g.old_price,g.sales_volume,g.fx_profit_money,g.virtual_volume,g.shop_id,g.ren_good_id FROM __PREFIX__order_detail o,__PREFIX__goods g WHERE o.order_id='$order_id' and o.goods_id=g.goods_id";
		$list=M()->query($sql);
		...
		if($list!==false)
		{
			...
			for($i=0;$i<$num;$i++)
			{
				...
				//属性字符串
                $list[$i]['sku_str']='';
				if($list[$i]['sku'] && $list[$i]['shop_id'] ==0) {
					...
					if($GoodsSkuMsg['img']) {
						$list[$i]['img']= (is_url($GoodsSkuMsg['img']) ? $GoodsSkuMsg['img'] : WEB_URL . $GoodsSkuMsg['img']);
					}
				...
				}else if ($list[$i]['sku'] && $list[$i]['shop_id'] >0)
                {
                    $shopOptionModel = new ShopGoodsOptionModel();
                    $goods_option = $shopOptionModel->getGoodsOptionById($list[$i]['ren_good_id'],$list[$i]['sku']);
                    $list[$i]['sku_str']=empty($goods_option)?'':$goods_option['title'];
                }

                // 查询是否礼包商品，是则不能退款标志
                $goodsMsg=$goods->where("goods_id='{$list[$i]['goods_id']}'")->find();
                $list[$i]['is_gift_goods'] = $goodsMsg['is_gift_goods'];
                if ($list[$i]['img']) {
