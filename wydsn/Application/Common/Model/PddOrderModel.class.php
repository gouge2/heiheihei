...
class PddOrderModel extends Model
{
	...
	public function treat($order_sn,$money)
	{
		...
		if($msg)
		{
            ...
            $userBrowse = $UserBrowseModel->getUserBrowse($msg['goods_id'], $uid);
            if ($userBrowse && strtotime($userBrowse['create_time'] . " +30 minutes") - time() > 0) {
                $HostTreatModel = new \Common\Model\HostTreatModel("pdd");
                return $HostTreatModel->submit($userBrowse['host_id'], "order_sn", $order_sn, $money);
            }
				...
				if(!$User->create($data_user))
				{
					...
					if($res_balance!==false and $res_record!==false and $res_order!==false)
					{
						...
						if($UserMsg['referrer_id'] and ($UserMsg['group_id']=='1' or $UserMsg['group_id']=='2'))
						{
							...
							$referrerMsgGroup = $UserGroup->getGroupMsg($referrerMsg['group_id']);
							//不同会员组，直接推荐佣金不同
							switch ($referrerMsg['group_id'])
							{
								...
								case 1:
									//总佣金12%
									$referrer_money=$money*$referrerMsgGroup['referrer_rate']/100;
									//团队中运营商、合伙人会员数
									$is_vip=0;
									break;
								//总监
							...
							if($res_balance_r!==false and $res_record_r!==false) {
								...
								if($referrerMsg['referrer_id'])
								{
									...
									switch ($referrerMsg2['group_id'])
									{
										...
										case 1:
											//无佣金
											$referrer_money2=$money*$referrerMsgGroup['referrer_rate2']/100;
											break;
										//总监
										case 2:
											//总佣金5%
...
//                                                            $res_reward_rt=$TeamRewardsLog->addLog($referrer_tid, $uid, $order_sn, 'pdd',$is_vip+1);
	...
	public function treat_tmp($order_sn,$money)
	{
		...
		if($msg)
		{
			...
			if($res_exist)
			{
                ...
                if ($userBrowse && strtotime($userBrowse['create_time'] . " +30 minutes") - time() > 0) {
                    $HostTreatModel = new \Common\Model\HostTreatModel("pdd", true);
                    return $HostTreatModel->submit($userBrowse['host_id'], "order_sn", $order_sn, $money);
                }
