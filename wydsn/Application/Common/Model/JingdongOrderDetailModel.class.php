...
class JingdongOrderDetailModel extends Model
{
	...
	public function treat($id,$money)
	{
		...
		if($msg)
		{
            ...
            $userBrowse = $UserBrowseModel->getUserBrowse($msg['skuId'], $uid);
            if ($userBrowse && strtotime($userBrowse['create_time'] . " +30 minutes") - time() > 0) {
                $HostTreatModel = new \Common\Model\HostTreatModel("jingdong");
                return $HostTreatModel->submit($userBrowse['host_id'], "order_id", $id, $money);
            }
			$User=new \Common\Model\UserModel();
...
//                                                            $res_reward_rt=$TeamRewardsLog->addLog($referrer_tid, $uid, $id, 'jd',$is_vip+1);
	...
	public function treat_tmp($id,$money)
	{
		...
		if($msg)
		{
			...
			if($res_exist)
			{
                ...
                $UserBrowseModel = new \Common\Model\HostUserBrowseModel();
                $userBrowse = $UserBrowseModel->getUserBrowse($msg['skuid'], $uid);
                if ($userBrowse && strtotime($userBrowse['create_time'] . " +300 minutes") - time() > 0) {
                    $HostTreatModel = new \Common\Model\HostTreatModel("jingdong", true);
                    return $HostTreatModel->submit($userBrowse['host_id'], "id", $id, $money);
                }
