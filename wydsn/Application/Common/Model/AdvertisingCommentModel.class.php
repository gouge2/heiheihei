...
class AdvertisingCommentModel extends Model
{
    //验证规则
    protected $_validate =array(
        array('short_id','require','广告标识不能为空！',self::EXISTS_VALIDATE),      //存在验证，必填
    ...
    public function getListData($uid, $where, $limit, $page, $platform = '', $parent_id = 0, $sort = ' sort desc,id desc ')
    {
        ...
        if ($list) {
            ...
            $AdvertisingCommentPraise = new \Common\Model\AdvertisingCommentPraiseModel();
            ...
            // 点赞列表
            $scp_list = $uid ? $AdvertisingCommentPraise->where(['comment_id' => ['in', $cid_arr], 'user_id' => $uid])->getField('comment_id', true) : [];
