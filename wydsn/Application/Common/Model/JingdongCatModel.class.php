...
class JingdongCatModel extends Model
{
	//验证规则
	...
	public function getGoodsCatList($is_show='')
	{
		...
		$cat=$this->where($where)->order('sort desc,jingdong_cat_id asc')->select();
		if($cat!==false)
		{
			$list =$this->rule($cat);
			return $list;
	...
	public function getParentList($is_show='')
	{
		...
		$list=$this->where($where)->order('sort desc,jingdong_cat_id asc')->select();
		if($list!==false)
		{
			return $list;
		}else {
	...
	public function getSubListByParent($pid,$order='asc',$is_show='')
	{
		...
		$list=$this->where($where)->order("sort desc,jingdong_cat_id $order")->select();
		if($list!==false)
		{
			return $list;
		}else {
	...
	public function getSubList($taobao_cat_id,$order='asc',$is_show='')
	{
		...
		$cat=$this->where($where)->order("sort desc,jingdong_cat_id $order")->select();
	...
	/**
	 * 获取商品分类信息
	 * @param int $jingdong_cat_id:商品分类ID
	 * @return array|boolean
	 */
	public function getMsg($jingdong_cat_id)
	{
		$msg=$this->where("jingdong_cat_id='$jingdong_cat_id'")->find();
		if($msg!==false)
		{
			return $msg;
		}else {
	...
	static public function rule($cate , $lefthtml = '— ' , $pid=0 , $lvl=0, $leftpin=0 )
	{
		...
		foreach ($cate as $v)
		{
			if($v['pid']==$pid)
			{
				...
				$arr[]=$v;
				$arr= array_merge($arr,self::rule($cate,$lefthtml,$v['jingdong_cat_id'],$lvl+1 , $leftpin+20));
			}
		}
		return $arr;
	}
