...
class UserBalanceRecordModel extends Model
{
	...
	protected $_validate =array(
			...
			array('pay_method',array('alipay','wxpay','int_wx','int_ali','paypal'),'支付方式类型不正确！',self::VALUE_VALIDATE,'in'),  //值不为空的时候验证，支付方式只能是 alipay支付宝 wxpay微信支付 国际微信 国际支付宝 paypal
	);
	...
	public function getActionZh($action)
	{
	    switch ($action) {
            ...
            case 'bonus3':
                ...
                break;
			case 'll_add':
				$action_zh 		= GIFT_MONEY_CN .'充值';
				$action_symbol 	= '-';
				break;
			case 'deer_take':
				$action_zh 		= GIFT_DEER_CN .'提取';
				$action_symbol 	= '+';
				break;
            case 'self':
                $action_zh='自营返利';
     ...
     * @return string
     */
    public function userCommissionStatistics($uid)
    {
        //获取用户累计结算收益
