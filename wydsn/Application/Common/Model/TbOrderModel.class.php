...
class TbOrderModel extends Model
{
	...
	public function pullOrder($query_type,$tk_status,$order_scene,$start_time,$end_time,$member_type,$page_no,$page_size,$position_index,$jump_type)
	{
	    ...
	    $User = new \Common\Model\UserModel();
        //注意：只要没有走确认收货的订单状态，就不会有total_commission_fee 和commission  #如果开启了大淘客订单，走大淘客的接口
        if(DTK_SEARCH_ORDER ==1)
        {
            Vendor('dataoke.dtk','','.class.php');
            $tbk=new \dtk();
            $results=$tbk->dtkGetOrderList($query_type,$tk_status,$order_scene,$start_time,$end_time,$member_type,$page_no,$page_size,$position_index,$jump_type);
        }else{
	...
	public function treat($trade_id,$money)
	{
		...
		if($msg) {
            ...
            $userBrowse = $UserBrowseModel->getUserBrowse($msg['num_iid'], $uid);
            if ($userBrowse && strtotime($userBrowse['create_time'] . " +30 minutes") - time() > 0) {
                $HostTreatModel = new \Common\Model\HostTreatModel("tb");
                return $HostTreatModel->submit($userBrowse['host_id'], "trade_id", $trade_id, $money);
            }
			...
			if($groupMsg and $UserMsg) {
				...
				writeLog(json_encode(['money'=>$money,'userGroup'=>$groupMsg,'uid'=>$uid]),'commission_log');
				//是否购物
				$old_is_buy=$UserMsg['is_buy'];
				//判断用户是否已领取过0元购
				$subsidy_amount=0;
	...
	public function treat_tmp($trade_id,$money)
	{
		...
		if($msg and $msg['tk_status']!='13') {
			...
			if($res_exist) {
                ...
                //判断用户是否买过0元购商品
                if($UserMsg['is_buy_free_goods']=='N'){
                    //未购买
                    //判断商品是否在0元购商品中
                    $TbGoodsFree=new \Common\Model\TbGoodsFreeModel();
                    $goods_id=$msg['num_iid'];
                    ...
                    if($goodsFreeMsg['subsidy_amount']>0){
                        ...
                        if(!$User->create($data_user)) {
                            ...
                            $res_balance=$User->where("uid='$uid'")->save($data_user);
                            //保存余额变动记录-预估
                            $res_record=$UserBalanceRecordTmp->addLog($uid,$subsidy_amount,$action='tbk_free',$trade_id,$type='1',$msg['create_time']);
