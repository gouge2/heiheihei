...
class GoodsModel extends Model
{
    ...
    public function getGoodsMsg($goods_id)
    {
        ...
        if ($msg !== false && !empty($msg)) {
            ...
            if ($msg['img']) {
                $msg['img'] = (is_url($msg['img']) ? $msg['img'] : WEB_URL . $msg['img']);
            }
            if ($msg['tmp_img']) {
                $msg['tmp_img'] = (is_url($msg['tmp_img']) ? $msg['tmp_img'] : WEB_URL . $msg['tmp_img']);
            }
            return $msg;
        } else {
            return false;
    ...
    {
        $goods = $this->getGoodsMsg($id);
        $shopGoodsSpecModel = new \Common\Model\ShopGoodsSpecModel();
        $shopGoodsOptionModel = new \Common\Model\ShopGoodsOptionModel();
        $shopGoodsSpecItemModel = new \Common\Model\ShopGoodsSpecItemModel();
        $specs_info = ['spec'=>[],'option'=>[]];
        if($goods)
        {
            $specs = $shopGoodsSpecModel->getSpec($goods['ren_good_id']);
            foreach($specs as &$spec) {
                #获取商品规格
                $spec_info = $shopGoodsSpecItemModel->getGoodsSpecItem($spec['id']);
                $spec['spec_list']=$spec_info;
            }
            #获取商品属性
            $options = $shopGoodsOptionModel->getGoodsOptionBySpecId($goods['ren_good_id']);
            $specs_info['option']=$options;
            $specs_info['spec']=$specs;
            return empty($specs_info)?[]:$specs_info;
        }
    ...
    /**
     * 购物车商品数据重组
     * @param $data     购物车json数据
     */
    public function sysShopCartGoods($data)
    {
        $goodslist_str=str_replace('\\', '', $_POST['goodslist']);
        $goodslist=json_decode($goodslist_str,true);
        $shopcartModel=new \Common\Model\ShopcartModel();
        $goodsModel = new \Common\Model\GoodsModel();
        $arr = [];
        $k = 0;
        $gift = [];
        foreach ($goodslist as $l) {
            $shopcartMsg=$shopcartModel->where("id='$l[shopcart_id]'")->find();
            $goods_id=$shopcartMsg['goods_id'];
            $goods_num=$l['goods_num'];
            $goodsMsg=$goodsModel->getGoodsMsg($goods_id);
            if($goodsMsg['is_gift_goods'] =='Y')
            {
                $arr[-1]['goods_list'][$k]['goods_num']=$goods_num;
                $arr[-1]['goods_list'][$k]['goods_id']=$goods_id;
                $arr[-1]['goods_list'][$k]['is_gift']=$goodsMsg['is_gift_goods'];
                $arr[-1]['goods_list'][$k]['goods_sku'] = $shopcartMsg['goods_sku'];
                $arr[-1]['goods_list'][$k]['shop_id']=$goodsMsg['shop_id'];
                $arr[-1]['goods_list'][$k]['remake']=$l['remake'];
            }else{
                $arr[$goodsMsg['shop_id']]['goods_list'][$k]['goods_num']=$goods_num;
                $arr[$goodsMsg['shop_id']]['goods_list'][$k]['goods_id']=$goods_id;
                $arr[$goodsMsg['shop_id']]['goods_list'][$k]['is_gift']=$goodsMsg['is_gift_goods'];
                $arr[$goodsMsg['shop_id']]['goods_list'][$k]['goods_sku'] = $shopcartMsg['goods_sku'];
                $arr[$goodsMsg['shop_id']]['goods_list'][$k]['shop_id']=$goodsMsg['shop_id'];
                $arr[$goodsMsg['shop_id']]['goods_list'][$k]['remake']=$l['remake'];
            }
            array_push($gift,$goodsMsg['is_gift_goods']);
            #新增一个判断
            $k++;
        }
        return array('list'=>$arr,'goods'=>$gift);
    }
}
?>
