...
class UserModel extends Model
{
	...
	public function checkToken($token)
	{
		...
		if ($msg) {
			...
			// 用户token失效
			$res = [
				'code' => $this->ERROR_CODE_USER['USER_TOKEN_DISABLED'],
				'msg'  => $this->ERROR_CODE_USER_ZH[$this->ERROR_CODE_USER['USER_TOKEN_DISABLED']]
			];
		}
	 ...
	 * 检查会员权限
	 */
	public function checkAuthority($type, $uid)
	{
		$res = [
			'code' 	=> $this->ERROR_CODE_USER['DO_NOT_HAVE'],
			'msg' 	=> $this->ERROR_CODE_USER_ZH[$this->ERROR_CODE_USER['DO_NOT_HAVE']]
		];

		if ($type &&  $uid) {
			$group_id 	= $this->where(['uid' => $uid])->getField('group_id');

			// 判断权限
			if ($group_id) {
				$live_power 	= USER_LIVE_POWER ? explode(',', USER_LIVE_POWER) : [];
				$short_power 	= USER_SHORT_POWER ? explode(',', USER_SHORT_POWER) : []; 
				$live_in 		= in_array($group_id, $live_power) ? true : false; 
				$short_in 		= in_array($group_id, $short_power) ? true : false;

				// 短视频权限  直播权限
				if (($type == 'short' && $short_in) || ($type == 'live' && $live_in))  {		
					$res = [
						'code' 	=> $this->ERROR_CODE_COMMON['SUCCESS'],
    ...
    /**
     * 获取邀请人列表
     * @param array $path 团队路径
     * @param bool $need_vip 是否只需要vip
     * @param int $limit 数据个数
     * @return array
     */
	public function getReferrer($path, $need_vip = false, $offset = 0) {
	    if (empty($path)) {
	        return array();
        }
        // 团队路径
	    ...
	    $where = "uid in ($path)";
	    if ($need_vip) {
	        $where .= " and group_id in (3,4)";
        }
        $teamList = $this->where($where)->order('uid desc')->limit($offset, 2 - $offset)->select();
	...
	public function getUserDetail($uid)
	{
		...
		if($msg!==false)
		{
			//会员账号
			if($msg['phone']) {
				$account=substr_replace($msg['phone'],'****',3,4);
			...
			if($msg['group_id']){
			    ...
			    $msg['group_icon']=$groupMsg['level_icon'];
			}
			//会员详情
	...
	public function getDrawBalance($uid)
	{
		...
		$amount_month=$UserBalanceRecord->where("user_id='$uid' and status='2' and date_format(pay_time,'%Y-%m')=date_format(now(),'%Y-%m') and action in ('tbk','tbk_r','tbk_r2','tbk_rt','jd','jd_r','jd_r2','jd_rt','pdd','pdd_r','pdd_r2','pdd_rt','vip','vip_r','vip_r2','vip_rt')")->sum('money');
	                        ...
	                        for($i=3;$i<$team_num;$i++) {
	                            if($is_vip<2) {
	                                ...
	                                if($referrer_tgid=='3' or $referrer_tgid=='4') {
                                        ...
                                        if($is_vip==0)
                                        {
                                            $referrer_money_team=$money*(defined('TEAM_REWARD1')?TEAM_REWARD1:0)/100;
                                        }else {
                                            $referrer_money_team=$money*(defined('TEAM_REWARD2')?TEAM_REWARD2:0)/100;
                                        }
	                                    //四舍五入，保留2位
...
//                                            $res_reward_rt=$TeamRewardsLog->addLog($referrer_tid, $userMsg['uid'], $order_id, 'tb',$is_vip+1);
                                            if($res_balance_rt!==false and $res_record_rt!==false /*and $res_reward_rt!==false*/)
	                                        {
	                                            //团队运营商、合伙人人数+1
	                                            $is_vip+=1;
...
//															$referrer_money_team=$money*0.05;
	                                            ...
	                                            if($referrer_money_team>0) {
                                                    ...
                                                    $TeamRewardsLog=new \Common\Model\TeamRewardsLogModel();
                                                    $res_reward_rt=$TeamRewardsLog->addLog($referrer_tid, $userMsg['uid'], $order_id, 'tb',$is_vip+1);
	                                                if($res_record_rt!==false and $res_reward_rt!==false) {
	                                                    //团队运营商、合伙人人数+1
	                                                    $is_vip+=1;
	                                                    continue;
	                                                }else {
	 ...
	 * 获取用户佣金比例
	 */
	public function getUserFeeRatio($token, &$group_id, $type = 'user')
	{
		$UserGroup 	= new \Common\Model\UserGroupModel();

		// 用户的佣金比例
		if ($type == 'user') {
			$group_id   = $token ? $this->where("token='{$token}'")->getField('group_id') : 1;
			$groupMsg 	= $UserGroup->getGroupMsg($group_id);
			$fee        = $groupMsg['fee_user'];

		// 最高佣金比例
		} elseif ($type == 'high') {
			$high       = $UserGroup->field('fee_user')->order('fee_user desc')->find();
			$fee        = $high['fee_user'];
		}
	...
	public function getUserId($token)
	{
		...
		if ($token) {
			$id 	= $this->where("token='{$token}'")->getField('uid');
			$uid  	= $id ? $id : 0;
		}
     ...
     */
	public function getJdPid($token){
        $jd_pid  = 0;

        if ($token) {
            $id 	= $this->where("token='{$token}'")->getField('jd_pid');
            $jd_pid  	= $id ? $id : 0;
        }
    ...
    #获取两个上级并且用户组大于2的
    public function getUserInfoByGroup($uid,$arr=[],$count=0)
    {
        $user = $this->getUserMsg($uid);
        if($user)
        {
            if($user['group_id']>2)
            {
                array_push($arr,$user['uid']);
                $count++;
            }
            if($count<2)
            {
                return $this->getUserInfoByGroup($user['uid'],$arr,$count);
            }
        }
        return $arr;
    }
    ...
    #获取相应的下级直播
    public function getHostUserLevel1($uid,$arr)
    {
        $user = $this->getUserMsg($uid);
        if($user)
        {
            #获取所有下级
            $ref_list = $this->where("referrer_id={$uid}")->field('uid')->select();
            for ($i=0;$i<count($ref_list);$i++)
            {
                array_push($arr,$ref_list[$i]['uid']);
            }
        }
        return $arr;
    }
    ...
    #获取相应的间推直播
    public function getHostUserLevel2($uid)
    {
        $member = $this->field('referrer_id,uid')->select();
        return $this->getMemberTeamId($member,$uid);
    }
     ...
     * @return int[]
     */
    protected function getMemberTeamId($members, $id ){
        $Teams = array(0);//最终结果
        ...
        do{
            ...
            foreach ($mids as $v) {
                foreach($members as $k => $memeber) {
                    if($memeber['referrer_id'] == $v){
                        if($count>0)
                        {
                            $Teams[]        = $memeber['uid'];//找到我的下级立即添加到最终结果中
                        }
                        $othermids[]    = $memeber['uid'];//将我的下级id保存起来用来下轮循环他的下级
