...
class IndexController extends Controller
{
    ...
    //修改代理商是否开启
    public function agentSwitch()
    {
        if($_SESSION['agent_id']!='') {
            $agent_id=$_SESSION['agent_id'];
            $User=new \Common\Model\UserModel();
            $userMsg=$User->where("uid=$agent_id")->field('agent_switch')->find();
            $this->assign('msg',$userMsg);
            if($_POST) {
                $old_agent_switch=I('post.old_agent_switch');
                $agent_switch=I('post.agent_switch');
                if ($old_agent_switch==$agent_switch){
                    $this->display();
                }else{
                    $data=array(
                    ...
                    if ($agent_switch=='Y'){
                        ...
                        $bannerList=$Banner->where("cat_id=1 and agent_id={$agent_id}")->select();
                        if ($bannerList){
                            //判断是否有显示状态的首页广告图
                            $bannerShowList=array_column($bannerList,'is_show');
                            if (in_array('Y',$bannerShowList)){
                                //判断是否设置了分享海报
                                $shareList=$Banner->where("cat_id=4 and agent_id={$agent_id}")->select();
                                if ($shareList){
                                    //判断是否有显示状态的分享海报
                                    $shareShowList=array_column($shareList,'is_show');
                                    if (in_array('Y',$shareShowList)){
                                        $res=$User->where("uid={$agent_id}")->save($data);
                                        if ($res!==false){
                                            $this->success('修改成功');
                                        }else{
                                            $this->error('修改失败');
                                        ...
                                        $this->error('请至少设置一张分享海报为显示状态');
                                    }
                                }else{
                                    $this->error('尚未设置分享海报');
                                }
                            }else{
                                $this->error('请至少设置一张首页广告图为显示状态');
                            }
                        }else{
                            $this->error('尚未设置首页广告图');
                        }
                    }else{
                        $res=$User->where("uid={$agent_id}")->save($data);
                        if ($res!==false){
