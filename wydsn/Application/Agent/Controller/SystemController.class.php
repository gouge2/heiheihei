...
class SystemController extends AuthController
{
    public function index_show()
    {
        ...
        $vip_order_finished_num=$vip_order_pay_num=$vip_order_today_num=$vip_order_month_num=0;
        ...
        if($all_uid){
            ...
            //已结算订单
            $vip_order_finished_num=$VipOrder->where("settled=2 and user_id in ($all_uid)")->count();
            //已付款订单
            $vip_order_pay_num=$VipOrder->where("orderSubStatusName in ('已下单','已付款','已签收','待结算') and user_id in ($all_uid)")->count();
            //今日订单
            $vip_order_today_num=$VipOrder->where("date_format(orderTime/1000,'%Y-%m-%d')=date_format(now(),'%Y-%m-%d') and user_id in ($all_uid)")->count();
            //本月订单
            $vip_order_month_num=$VipOrder->where("date_format(orderTime/1000,'%Y-%m')=date_format(now(),'%Y-%m') and user_id in ($all_uid)")->count();
        }
