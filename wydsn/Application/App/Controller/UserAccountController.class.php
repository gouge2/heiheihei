...
class UserAccountController extends AuthController
{
    ...
    public function loginSetup()
    {
        ...
        $data=array(
            ...
            'seconds_verify'=>SECONDS_VERIFY,
            'ty_link'=>TY_LINK,
//            'kd_type'=>KD_TYPE,
            'kd_key'=>KD_KEY,
        );
        $facebook_type = defined('FACEBOOK_TYPE') ? FACEBOOK_TYPE : '';
        $twitter_type = defined('TWITTER_TYPE') ? TWITTER_TYPE : '';
        $data['tripartite'] =defined('LOGIN_METHODS') ? LOGIN_METHODS : '';//登录方式
        if (!$twitter_type && strpos($data['tripartite'],'twi_login') !== false) {
            $data['tripartite'] = str_replace(',twi_login','',$data['tripartite']);
        }
        if (!$facebook_type && strpos($data['tripartite'],'face_login') !== false) {
            $data['tripartite'] = str_replace(',face_login','',$data['tripartite']);
        }
        if ($platform != 'ios' &&  strpos($data['tripartite'],'appl_login') !== false) {
            $data['tripartite'] = str_replace(',appl_login','',$data['tripartite']);
        }
        $res=array(
...
//						}
                        ...
                        if($refer){
                            $res_referrer=$User->where("phone='$refer' or username='$refer' or auth_code='$refer'")->find();
                            if($res_referrer) {
                                $referrer_id=$res_referrer['uid'];
                            }else {
                                //推荐人不存在
									...
									if($res_detail!==false and $res_path!==false and $res_point_record!==false and $res_code!==false)
									{
										...
										if($referrer_id) {
											if(POINT_RECOMMEND_REGISTER>0) {
												...
												//推荐人积分存量
												$allpoint = isset($res_referrer) ? $res_referrer['point']+POINT_RECOMMEND_REGISTER : 0;
												//保存积分变动记录
			...
			if($res_code['code']!=200) {
				...
				if($UserMsg) {
					...
					if(!$User->create($data)) {
						...
						$avatar = str_replace("http://", "https://",$avatar);    //转化为https
						//性别
						$sex=trim(I('post.sex'));
						//保存用户详情
						$data_detail=array(
