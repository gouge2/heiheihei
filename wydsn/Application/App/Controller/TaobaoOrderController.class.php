...
//							$referrerList2=$User->where("referrer_id in ($all_uid)")->field('uid')->select();
                    ...
                    case 4:
                        ...
                        $res_reward_list=$TeamRewardsLog->getRecordList($uid,'tb');
                        if($res_reward_list)
                        {
                        ...
                        if($all_order_id)
                        {
                            ...
                            $where.=" and trade_id in ($all_order_id)";
                        }else {
                            $where='';
                            $data=array(
                                'list'=>array()
...
//                        $groupMsg=$UserGroup->getGroupMsg($UserMsg['group_id']);
						...
						if($type=='2')
						{
							...
							{
								$list[$i]['commission']=$list[$i]['pub_share_pre_fee']*$groupMsg['referrer_rate']/100;;
							}
						}elseif ($type=='3'){
                            //间推订单佣金
                            if(empty($list[$i]['commission']) or $list[$i]['commission']=='0.00' or $list[$i]['commission']==$list[$i]['total_commission_fee'])
                            {
                                $list[$i]['commission']=$list[$i]['pub_share_pre_fee']*$groupMsg['referrer_rate2']/100;;
                            }
                        }elseif ($type=='4'){
                            //间推订单佣金
                            if(empty($list[$i]['commission']) or $list[$i]['commission']=='0.00' or $list[$i]['commission']==$list[$i]['total_commission_fee'])
                            {
                                //获取团队分红订单记录
                                $TeamRewardsLog=new \Common\Model\TeamRewardsLogModel();
                                $res_reward=$TeamRewardsLog->getRecordMsg($uid,$list[$i]['trade_id']);
                                if ($res_reward['rewards_level']==1){
                                    $list[$i]['commission']=$list[$i]['pub_share_pre_fee']*TEAM_REWARD1/100;
                                }else{
                                    $list[$i]['commission']=$list[$i]['pub_share_pre_fee']*TEAM_REWARD2/100;
                                }
                                //四舍五入
