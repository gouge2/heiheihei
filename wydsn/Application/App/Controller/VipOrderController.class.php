...
class VipOrderController extends AuthController
{
     ...
     * @param string $order_status:订单状态描述（ 已下单、已付款、已签收、待结算、已结算、已失效）
     ...
     * @return @param data->list:唯品会订单列表
     */
        ...
        if(trim(I('post.token')))
        {
            ...
            if($res_token['code']!=0)
            {
                ...
                if(trim(I('post.order_sn'))!=='')
                {
                    ...
                    $where.=" and orderSn='$order_sn'";
                }
                if(trim(I('post.order_status'))!=='')
                {
                    $order_status=trim(I('post.order_status'));
                    $where.=" and orderSubStatusName='$order_status'";
                }
                if(trim(I('post.type')))
                {
...
//                            $referrerList2=$User->where("referrer_id in ($all_uid)")->field('uid')->select();
                    ...
                    case 4:
                        ...
                        $res_reward_list=$TeamRewardsLog->getRecordList($uid,'vip');
                        if($res_reward_list)
                        {
                        ...
                        if($all_order_id)
                        {
                            ...
                            $where.=" and orderSn in ($all_order_id)";
                        }else {
                            $where='';
                            $data=array(
                                'list'=>array()
...
//                            $UserGroup=new \Common\Model\UserGroupModel();
//                            $groupMsg=$UserGroup->getGroupMsg($UserMsg['group_id']);
//                            $list[$i]['commission']=$list[$i]['commission']*$groupMsg['fee_user']/100;
//                        }
                        ...
                        {
                            //直推订单佣金
                            if(empty($list[$i]['commission']) or $list[$i]['commission']=='0.00' or $list[$i]['commission']==$list[$i]['vipcommission'])
                            {
                                $list[$i]['commission']=$list[$i]['vipcommission']*$groupMsg['referrer_rate']/100;
                                //四舍五入
                            ...
                            if(empty($list[$i]['commission']) or $list[$i]['commission']=='0.00' or $list[$i]['commission']==$list[$i]['vipcommission'])
                            {
                                $list[$i]['commission']=$list[$i]['vipcommission']*$groupMsg['referrer_rate2']/100;
                                //四舍五入
                                $list[$i]['commission']=round($list[$i]['commission'], 2);
                            }
                        }elseif ($type=='4'){
                            ...
                            if(empty($list[$i]['commission']) or $list[$i]['commission']=='0.00' or $list[$i]['commission']==$list[$i]['vipcommission'])
                            {
                                //获取团队分红订单记录
                                $TeamRewardsLog=new \Common\Model\TeamRewardsLogModel();
                                $res_reward=$TeamRewardsLog->getRecordMsg($list[$i]['orderSn']);
                                if ($res_reward['rewards_level']==1){
                                    $list[$i]['commission']=$list[$i]['vipcommission']*TEAM_REWARD1/100;
                                }else{
                                    $list[$i]['commission']=$list[$i]['vipcommission']*TEAM_REWARD2/100;
                                }
                                //四舍五入
                                $list[$i]['commission']=round($list[$i]['commission'], 2);
                            }
                            ...
                            if(empty($list[$i]['commission']) or $list[$i]['commission']=='0.00' or $list[$i]['commission']==$list[$i]['vipcommission'])
                            {
                                $list[$i]['commission']=$list[$i]['vipcommission']*$groupMsg['fee_user']/100;
                                //四舍五入
                                $list[$i]['commission']=round($list[$i]['commission'], 2);
                            }
                        }
