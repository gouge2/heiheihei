...
class TbkController extends AuthController
{
	...
	public function getGoodsMsg()
	{
		if(trim(I('post.num_iid'))) {
			...
			$res_tbk=$tbk->getItemDetail($num_iid,'2',$ip,$pid,$relationId,$version);
            #实际佣金
            $fx_commission	 	=$res_tbk['data']['commission'];
            #效验是否存在商品列表
            $shortLiveModel = new \Common\Model\ShortLiveGoodsModel();
            $goodsItem = $shortLiveModel->getOne(['from'=>'tb','goods_id'=>$num_iid]);
            $is_has = 0;
            if($goodsItem)
            {
                $is_has = 1;
            ...
            $res_tbk['data']['commission'] = $userCommission['userHasCommission'];
            $res_tbk['data']['commission_host'] = $userCommission['hostUserCommission'];
            $res_tbk['data']['commission_vip'] = $userCommission['vipHasCommission'];
	...
	public function searchTkl()
	{
	    ...
	    if ($tkl) {
            ...
            $res_dtk = $DaTaoKe->amoyPasswordAnalysis($tkl);
                if ($res_dtk['data']['goodsId']) {
                    $data['num_iid'] = $res_dtk['data']['goodsId'];//商品ID
                } else {
                    //使用淘宝联盟官方接口
                    //解析淘口令
                    Vendor('tbk.tbk','','.class.php');
	 ...
	 * @return @param data->list:淘宝客商品列表
	 */
	public function getHotGoodsList($types = '')
	{
		$token     		= trim(I('post.token'));
		$agent_id 		= trim(I('post.agent_id'));
        if (empty($types)) {
            $per = I('post.per/d', self::$limit);
            $p = I('post.p/d', self::$page);
        } else {
            $per = I('post.back/d', self::$limit);
            $p = I('post.min_id/d', self::$page);
        }
	    ...
	    if ($list!==false) {
			...
			// 收藏列表
			$g_coll 		= $TbGoodsCollect->getUserCollect($uid, $gid_arr);

			foreach ($list as $key => $val) {
				$list[$key]['small_images'] = json_decode($val['small_images'], true);
				$list[$key]['is_collect'] 	= isset($g_coll[$val['goods_id']]) ? 'Y' : 'N';
			}
            if (empty($types)) {
     ...
     * @return @param data:返回数据
     */
    public function vyActivityInfo()
    {
        if(trim(I('post.token'))) {
            //判断用户身份
            $token = trim(I('post.token'));
            ...
            if ($res_token['code'] != 0) {
                ...
                $relationId = $userMsg['tb_rid'];
                $DaTaoKe = new \Common\Model\DaTaoKeModel();
                $data['channelID'] = $relationId;
                $result = $DaTaoKe->officialEventLink($data);
                $res = array(
                    'code' => $result['code'],
                    'msg' => $result['msg'],
                    'data' => $result['data']
