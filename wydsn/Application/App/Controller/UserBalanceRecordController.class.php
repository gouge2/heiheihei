...
use Common\Model\VipOrderModel;
...
class UserBalanceRecordController extends AuthController
{
        ...
        if(trim(I('post.token')) and trim(I('post.type')))
        {
            ...
            if($res_token['code']!=0)
            {
                ...
                switch ($type)
                {
                    //佣金收入
                    case '1':
                        $where.=" and action in('tbk','tbk_r','tbk_r2','tbk_rt','tbk_refund','pdd','pdd_r','pdd_r2','pdd_rt','jd','jd_r','jd_r2','jd_rt','vip','vip_r','vip_r2','vip_rt')";
                        break;
                    //提现
                    case '2':
                ...
                //获取用户余额变动记录
                $list=$UserBalanceRecord->where($where)->field('id,user_id,money,all_money,pay_time,action,order_id')->page($page,$per)->order("id desc")->select();
                if($list!==false)
                {
                    $num=count($list);
                    ...
                    for ($i=0;$i<$num;$i++)
                    {
                        ...
                        if($list[$i]['action']=='draw'){
                            $draw_info=$UserDrawApply->getApplyDetail($list[$i]['order_id']);
                            $list[$i]['real_money'] = sprintf("%.2f",$draw_info['real_money']);
                        }
                        else{
                            $list[$i]['real_money'] = "";
                        }
	...
	public function statistics()
	{
		if(trim(I('post.token')))
		{
			...
			if($res_token['code']!=0)
			{
				...
				//获取用户累计结算收益
				$UserBalanceRecord=new \Common\Model\UserBalanceRecordModel();
				$amount=$UserBalanceRecord->where("user_id='$uid' and action in('tbk','tbk_r','tbk_r2','tbk_rt','pdd','pdd_r','pdd_r2','pdd_rt','jd','jd_r','jd_r2','jd_rt','vip','vip_r','vip_r2','vip_rt')")->sum('money');
				$amount=$amount/100;
	...
	public function statistics2()
	{
		if(trim(I('post.token')))
		{
			...
			if($res_token['code']!=0)
			{
				...
				//累计预估-京东
				$jd_amount=$JingdongOrderDetail->where("user_id='$uid' and validCode in (16,17,18)")->sum('estimateFee');
				$jd_amount=$jd_amount*$fee_user;
                //累计预估-唯品会
                $vip_amount=$VipOrder->where("user_id='$uid' and orderSubStatusName in ('已下单','已付款','已签收','待结算','已结算')")->sum('vipCommission');
                $vip_amount=$vip_amount*$fee_user;
                //保留2位小数，四舍五不入
				$amount=$tbk_amount+$pdd_amount+$jd_amount+$vip_amount;
				$amount=substr(sprintf("%.3f",$amount),0,-1);
				...
				//上月结算
				$amount_last_finish=$UserBalanceRecord->where("user_id='$uid' and status='2' and date_format(pay_time,'%Y-%m')=date_format(DATE_SUB(curdate(), INTERVAL 1 MONTH),'%Y-%m') and action in ('tbk','tbk_r','tbk_r2','tbk_rt','jd','jd_r','jd_r2','jd_rt','pdd','pdd_r','pdd_r2','pdd_rt','vip','vip_r','vip_r2','vip_rt')")->sum('money');
	...
	public function teamStatistics()
	{
		if(trim(I('post.token')))
		{
			...
			if($res_token['code']!=0)
			{
				...
				//累计预估-京东
				$jd_amount=$JingdongOrderDetail->where("user_id in ($team_uid) and validCode in (16,17,18)")->sum('estimateFee');
                //累计预估-唯品会
                $vip_amount=$VipOrder->where("user_id in ($team_uid) and orderSubStatusName in ('已下单','已付款','已签收','待结算','已结算')")->sum('vipCommission');
				//保留2位小数，四舍五不入
				$amount=$tbk_amount+$pdd_amount+$jd_amount+$vip_amount;
				$amount=substr(sprintf("%.3f",$amount),0,-1);
				...
				//上月结算
				$amount_last_finish=$UserBalanceRecord->where("user_id in ($team_uid) and status='2' and date_format(pay_time,'%Y-%m')=date_format(DATE_SUB(curdate(), INTERVAL 1 MONTH),'%Y-%m') and action in ('tbk','tbk_r','tbk_r2','tbk_rt','jd','jd_r','jd_r2','jd_rt','pdd','pdd_r','pdd_r2','pdd_rt','vip','vip_r','vip_r2','vip_rt')")->sum('money');
				$amount_last_finish=$amount_last_finish/100;
				//保留2位小数，四舍五不入
				$amount_last_finish=substr(sprintf("%.3f",$amount_last_finish),0,-1);
	...
	public function drawStatistics()
	{
		if(trim(I('post.token')))
		{
			...
			if($res_token['code']!=0)
			{
				...
				//本月淘宝客、拼多多、京东、唯品会收入
				$UserBalanceRecord=new \Common\Model\UserBalanceRecordModel();
				$amount_mouth=$UserBalanceRecord->where("user_id='$uid' and status='2' and date_format(pay_time,'%Y-%m')=date_format(now(),'%Y-%m') and action in ('tbk','tbk_r','tbk_r2','tbk_rt','pdd','pdd_r','pdd_r2','pdd_rt','jd','jd_r','jd_r2','jd_rt','vip','vip_r','vip_r2','vip_rt')")->sum('money');
				...
				//购买返利-不包含本月
				$amount_buy=$UserBalanceRecord->where("user_id='$uid' and status='2' and date_format(pay_time,'%Y-%m')!=date_format(now(),'%Y-%m') and action in ('tbk','pdd','jd','vip')")->sum('money');
				$amount_buy=$amount_buy/100;
				//保留2位小数，四舍五不入
				$amount_buy=substr(sprintf("%.3f",$amount_buy),0,-1);
				
				//推荐返利-不包含本月
				$amount_referrer=$UserBalanceRecord->where("user_id='$uid' and status='2' and date_format(pay_time,'%Y-%m')!=date_format(now(),'%Y-%m') and action in ('tbk_r','tbk_r2','tbk_rt','pdd_r','pdd_r2','pdd_rt','jd_r','jd_r2','jd_rt','vip_r','vip_r2','vip_rt')")->sum('money');
	...
	public function receiveBonus()
	{
		...
		if(trim(I('post.token')))
		{
			...
			$start = defined('START_TIME') ? START_TIME : '';
            $end = defined('END_TIME') ? END_TIME : '';
            $uid=$res_token['uid'];
            //判断用户是否已经领取过红包
            $UserBalanceRecord=new \Common\Model\UserBalanceRecordModel();
            ...
            if (!empty($start) || !empty($end)) {
                $now = date('Y-m-d');
                $res['start_time'] = defined('START_TIME') ? START_TIME : $now;
                $res['end_time'] = defined('END_TIME') ? END_TIME : $now;
                if ($end < $now) {
                    $res['code'] = 1;
                    $res['msg'] = '现金红包活动已经结束';
                    $res['money'] = $res_exist['money'] ?: 0;
                    $res['data'] = [];
...
//					//未领取，可以领取
					...
					if($userMsg['is_buy']=='Y')
					{
						//已购物的人领取固定的0.88元
						//红包金额
						$money = defined('RATED_AMOUNT') ? RATED_AMOUNT : '0.88';
					}else {
						//未购物，领取红包金额随机
                        $random_amount1 = defined( 'RANDOM_AMOUNT1') ? RANDOM_AMOUNT1 : '0.88';
                        $random_amount2 = defined( 'RANDOM_AMOUNT2') ? RANDOM_AMOUNT2 : '1.88';
                        $random_amount3 = defined( 'RANDOM_AMOUNT3') ? RANDOM_AMOUNT3 : '2.18';
                        $random_amount4 = defined( 'RANDOM_AMOUNT4') ? RANDOM_AMOUNT4 : '2.28';
                        $random_amount5 = defined( 'RANDOM_AMOUNT5') ? RANDOM_AMOUNT5 : '2.68';
                        $random_amount6 = defined( 'RANDOM_AMOUNT6') ? RANDOM_AMOUNT6 : '2.88';
                        $money_arr = array($random_amount1, $random_amount2, $random_amount3, $random_amount4, $random_amount5, $random_amount6);
						$i=rand(0, 5);
						//红包金额
						$money=$money_arr[$i];
					}
        ...
        $res['start_time'] = defined('START_TIME') ? START_TIME : $start_time;
        $res['end_time'] = defined('END_TIME') ? END_TIME : $end_time;
		echo json_encode ($res,JSON_UNESCAPED_UNICODE);
	}
     ...
     * @return @param msg:返回码说明
     */
    public function rebateText()
    {
        if (!defined('REBATE_METHOD') or !defined('REBATE_METHOD')){
            $res='每月25号结算上个月已结算订单佣金';
        } else {
            if (REBATE_METHOD==1){
                $res='每月'.REBATE_TIME.'号结算上个月已结算订单佣金';
            }else{
                $res='订单结算后'.REBATE_TIME.'天结算订单佣金';
            }
        }
