...
class WxNotifyController extends Controller
{
	...
	public function notify_app()
	{
		...
		if($res['return_code'] ==="SUCCESS" && $res['return_msg'] ==="OK")
		{
			...
			if (!$ret) {
				$ret = json_decode(json_encode(simplexml_load_string(str_replace('\\', '', $raw_xml), 'SimpleXMLElement', LIBXML_NOCDATA)), true);
			}
			...
			switch ($type)
			{
                ...
                //商城
                case 'bp':
                    $Order=new \Common\Model\BoodModel();
                    $res=$Order->treatOrder($order_num, 'wxpay');
					break;
				// 鹿角充值
				case 'll':
					$FillRecord     = new \Common\Model\FillRecordModel();
					$res 			= $FillRecord->treatFill($order_num, 'wxpay');
					break;
				default:
					break;
			}
