...
class HaodankuController extends AuthController
{
    ...
    public function getGoodsList()
    {
        if( trim(I('post.type')) ){
            ...
            if( trim(I('post.token')))
            {
                ...
                $uid = $User->order('group_id desc')->getField('uid');
            }
            ...
            {
                #佣金价
                $commission = $res['data'][$i]['itemendprice']*($res['data'][$i]['tkrates']/100);
                $is_has =0;
                $short_goods= $shortModel->getOne(['from'=>'tb','goods_id'=>$res[$i]['itemid']]);
                if($short_goods)
                {
                    $is_has =1;
                }
    ...
    /**
     * 根据前端url获取好单库数据
     * @param string $url:必填，凭借地址
     * @return array
     */
    public function getHaoDanKuForUrl()
    {
        $url='http://v2.api.haodanku.com'.trim(I('post.url'));
