...
class TbGoodsCollectController extends AuthController
{
	/**
	 * 收藏商品
	...
	public function getCollectList()
	{
		if(trim(I('post.token')))
		{
			...
			if($res_token['code']!=0)
			{
				...
				$uid=$res_token['uid'];
				$TbGoodsCollect=new \Common\Model\TbGoodsCollectModel();
				$goodslist=$TbGoodsCollect->where("user_id='$uid'")->page($page, $limit)->order('collect_time desc')->select();
				if($goodslist!==false)
				{
					$goods_allid='';
					...
					if($goods_allid)
					{
						...
						if($res['code']==0)
						{
							...
							for($i=0;$i<$num;$i++)
							{
								...
								$is_has = 0;
								$live_goods = $shortLiveModel->getOne(['from'=>'tb','goods_id'=>$list[$i]['num_iid']]);
								if($live_goods)
                                {
                                    $is_has=1;
                                }
                                $userCommission = $hostCommissionModel->getCommissionByUser($uid,$is_has,$list[$i]['commission'],$vipGroupEnd['id']);
                                $list[$i]['commission'] = $userCommission['userHasCommission'];
                                $list[$i]['commission_host'] = $userCommission['hostUserCommission'];
                                $list[$i]['commission_vip'] = $list[$i]['commission_high']  = $userCommission['vipHasCommission'];
							}
							$data=array(
									'list'=>$list
							);
