...
class VipCollectController extends AuthController
{
    /**
     * 收藏商品
    ...
    public function collect()
    {
        if(I('post.token') and I('post.goods_id'))
        {
            ...
            if($res_token['code']!=0)
            {
                ...
                //判断是否已收藏该商品
                $VipCollect=new \Common\Model\VipCollectModel();
                $res_c=$VipCollect->where("goods_id='$goods_id' and user_id='$uid'")->find();
                if($res_c)
                {
                    //已收藏该商品，请勿重复收藏
                    $res=array(
    ...
    public function cancelCollect()
    {
        if(I('post.token') and I('post.goods_id'))
        {
            ...
            if($res_token['code']!=0)
            {
                ...
                $goods_id=trim(I('post.goods_id'));
                $VipCollect=new \Common\Model\VipCollectModel();
                $res_del=$VipCollect->where("goods_id='$goods_id' and user_id='$uid'")->delete();
                if($res_del)
                {
                    $res=array(
                        'code'=>$this->ERROR_CODE_COMMON['SUCCESS'],
    ...
    public function getCollectList()
    {
        if(I('post.token'))
        {
            ...
            if($res_token['code']!=0)
            {
                ...
                $VipCollect=new \Common\Model\VipCollectModel();
                $goodslist=$VipCollect->where("user_id='$uid'")->order('id desc')->page($p,$per)->select();
                if($goodslist!==false)
                {
                    $goods_allid='';
    ...
    public function is_collect()
    {
        if(I('post.token') and I('post.goods_id'))
        {
            ...
            if($res_token['code']!=0)
            {
                ...
                $goods_id=trim(I('post.goods_id'));
                $VipCollect=new \Common\Model\VipCollectModel();
                $res_exist=$VipCollect->where("goods_id='$goods_id' and user_id='$uid'")->find();
                if($res_exist)
                {
                    $is_collect='Y';
                }else {
