...
//                            $referrerList2=$User->where("referrer_id in ($all_uid)")->field('uid')->select();
                    ...
                    case 4:
                        ...
                        $res_reward_list=$TeamRewardsLog->getRecordList($uid,'jd');
                        if($res_reward_list)
                        {
...
//                        }
                        ...
                        if($l['commission']>0)
                        {
                            ...
                            {
                                //直推订单佣金
                                if(empty($l['commission']) or $l['commission']=='0.00' or $l['commission']==$l['actualfee'])
                                {
                                    $commission=$l['actualfee']*$groupMsg['referrer_rate']/100;
                                    //四舍五入
                                ...
                                if(empty($l['commission']) or $l['commission']=='0.00' or $l['commission']==$l['actualfee'])
                                {
                                    $commission=$l['actualfee']*$groupMsg['referrer_rate2']/100;
                                    //四舍五入
                                    $commission=round($commission, 2);
                                }
                            }elseif ($type=='4'){
                                ...
                                if(empty($l['commission']) or $l['commission']=='0.00' or $l['commission']==$l['actualfee'])
                                {
                                    //获取团队分红订单记录
                                    $TeamRewardsLog=new \Common\Model\TeamRewardsLogModel();
                                    $res_reward=$TeamRewardsLog->getRecordMsg($l['order_id']);
                                    if ($res_reward['rewards_level']==1){
                                        $commission=$l['actualfee']*TEAM_REWARD1/100;
                                    }else{
                                        $commission=$l['actualfee']*TEAM_REWARD2/100;
                                    }
                                    //四舍五入
                                    $commission=round($commission, 2);
                                }
                                ...
                                if(empty($l['commission']) or $l['commission']=='0.00' or $l['commission']==$l['actualfee'])
                                {
                                    $commission=$l['actualfee']*$groupMsg['fee_user']/100;
                                    //四舍五入
                                    $commission=round($commission, 2);
                                }
                            }
                            ...
                            {
                                //直推订单佣金
                                if(empty($l['commission']) or $l['commission']=='0.00' or $l['commission']==$l['estimatefee'])
                                {
                                    $commission=$l['estimatefee']*$groupMsg['referrer_rate']/100;
                                    //四舍五入
                                    $commission=round($commission, 2);
                                }
                            }elseif ($type=='3'){
                                ...
                                if(empty($l['commission']) or $l['commission']=='0.00' or $l['commission']==$l['estimatefee'])
                                {
                                    $commission=$l['estimatefee']*$groupMsg['referrer_rate2']/100;
                                    //四舍五入
                                    $commission=round($commission, 2);
                                }
                            }elseif($type=='4'){
                                if(empty($l['commission']) or $l['commission']=='0.00' or $l['commission']==$l['estimatefee'])
                                {
                                    //获取团队分红订单记录
                                    $TeamRewardsLog=new \Common\Model\TeamRewardsLogModel();
                                    $res_reward=$TeamRewardsLog->getRecordMsg($uid, $l['order_id']);
                                    if ($res_reward['rewards_level']==1){
                                        $commission=$l['estimatefee']*TEAM_REWARD1/100;
                                    }else{
                                        $commission=$l['estimatefee']*TEAM_REWARD2/100;
                                    }
                                    //四舍五入
                                    $commission=round($commission, 2);
                                }
                        ...
                        //商品图片
                        $goods_id=$l['skuid'];
                        //$url = 'https://router.jd.com/api?v=1.0&method=jd.union.open.goods.promotiongoodsinfo.query&access_token=&app_key=33ec496b1861874a92c103baeb07b1b9&sign_method=md5&format=json&timestamp=2020-04-03 09:25:55&sign=FD7B316AD1AD6CF275D36B11F57FCCE1&param_json={"skuIds":"{$goods_id}"}';
                        $url="https://api.jd.com/routerjson/iteminfo?skuids=$goods_id";

                        $res_g = $JdUnion->getGoodsInfo($goods_id);
