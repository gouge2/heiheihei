...
class Poster_EweiShopV2Page extends AppMobilePage
{
    ...
    private function createPoster($goods = array(), $member = array()) {
        ...
        $path = IA_ROOT . "/addons/shop/data/poster_wxapp/goods/" . $_W['uniacid'] . "/";
        if (!is_dir($path)) {
            load()->func('file');
            mkdirs($path);
        }
        ...
        //字体
        $font = IA_ROOT . "/addons/shop/static/fonts/pingfang.ttf";
        if(!is_file($font)){
            $font = IA_ROOT . "/addons/shop/static/fonts/msyh.ttf";
        }
...
//        // 价格背景
//        $pricebgstr = file_get_contents(IA_ROOT . "/addons/shop/plugin/app/static/images/poster/goodsprice.png");
...
//        imagecopyresized($target, $pricebg, 32, 632, 0, 0, 686, 86, 686, 86);
        ...
        if ($goods['productprice'] !== "0.00"){
         ...
         // 二维码
        //$boxstr = file_get_contents(IA_ROOT . "/addons/shop/plugin/app/static/images/poster/goodsbox.png");
...
//        die;
    ...
    private function getImgUrl($filename) {
        global $_W;
        return $_W['siteroot'] . "addons/shop/data/poster_wxapp/goods/". $_W['uniacid']. "/". $filename. '?v=1.0';
    }
    ...
    public function getposter()
    {
        ...
        $path = IA_ROOT . "/addons/shop/data/toouch_loop/code/" . $_W['uniacid'] . '/';
        $target = $path . $filename;
        load()->func('file');
        mkdirs($path);
        if (is_file($target)) {
            $file = $_W['siteroot'] . "addons/shop/data/toouch_loop/code/" . $_W['uniacid'] . "/" . $filename . '?v=1.0';
