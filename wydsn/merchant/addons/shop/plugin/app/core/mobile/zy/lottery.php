...
class Lottery_EweiShopV2Page extends AppMobilePage
{
	public function get_list()
	{
		...
		$banner = empty($set['thumb'])?'../addons/shop/plugin/lottery/static/images/lottery_banner.png':$set['thumb'];
		...
		foreach ($lottery as $key => $value) {
			...
			if(empty($value['lottery_icon'])){
				if ($value['lottery_type'] == 1) {
					$lottery[$key]['lottery_icon'] = '../addons/shop/plugin/lottery/static/images/panicon.png';
				}
				else if ($value['lottery_type'] == 2) {
					$lottery[$key]['lottery_icon'] = '../addons/shop/plugin/lottery/static/images/guaicon.png';
				}
				else {
					if ($value['lottery_type'] == 3) {
						$lottery[$key]['lottery_icon'] = '../addons/shop/plugin/lottery/static/images/gridicon.png';
					}
				}
			}
		...
		foreach ($mylog as $key => $value) {
			...
			if (isset($lottery_data['credit'])) {
				...
				if (isset($lottery_data['coupon'])) {
					...
					$mylog[$key]['rewarded'] = 1;
					$mylog[$key]['icon'] = '../addons/shop/plugin/lottery/static/images/quan.png';
				}
			}
