...
class Op_EweiShopV2Page extends MerchWebPage
{
	...
	public function finish() 
	{
        ...
        #同步订单操作
        $ren_order = pdo_fetch("select * from ".tablename('order')." where ren_order_id=:id",array(":id"=>$item['id']));

        if($ren_order) {
            pdo_update('order', ['finish_time'=>date('Y-m-d H:i:s'),'status' => 4], ['ren_order_id' => $item['id']]);
        }
		show_json(1);
	}
	...
	public function sendcancel() 
	{
		...
		if ($_W['ispost']) 
		{
            ...
            if($ren_order) {
                pdo_update('order', ['logistics' => '', 'express_number' => '', 'deliver_time'=>'','status' => 2], ['ren_order_id' => $item['id']]);
            }
	...
	public function fetch() 
	{
        ...
        #同步发货状态
        pdo_update('order',['finish_time'=>date('Y-m-d H:i:s',time()),'status'=>4],['ren_order_id'=>$item['id']]);
		show_json(1);
	}
	public function send() 
	{
		...
		if ($_W['ispost']) 
		{
            ...
            if($ren_order)
            {
                $expressRen = m('express')->checkExpress(trim($_GPC['expresscom']));
                if($expressRen)
                {
                    pdo_update('order',['logistics'=>$expressRen,'express_number'=>$_GPC['expresssn'],'status'=>3],['ren_order_id'=>$item['id']]);
                }
            }
	...
	public function changeexpress() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			if (!(empty($expresssn))) 
			{
                ...
                #同步订单操作
                $ren_order = pdo_fetch("select * from ".tablename('order')." where ren_order_id=:id",array(":id"=>$id));
                ...
                if($ren_order)
                {
                    ...
                    if($expressRen)
                    {
                        pdo_update('order',['logistics'=>$expressRen,'express_number'=>$_GPC['expresssn'],'status'=>3],['ren_order_id'=>$id]);
                    }
                }

				plog('order.op.changeexpress', '修改快递状态 ID: ' . $id . ' 订单号: ' . $item['ordersn'] . ' 快递公司: ' . $expresscom . ' 快递单号: ' . $expresssn);
				show_json(1);
			}
			else 
			{
