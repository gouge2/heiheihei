...
//                            }
                ...
                #同意退款
                $orderInfo = pdo_fetch("select * from ".tablename("order")." where ren_order_id=:id",array(":id"=>$item['id']));
                if($orderInfo)
                {
                    pdo_update('order',['status'=>7,'refund_success_time'=>date('Y-m-d H:i:s')],['ren_order_id'=>$item['id']]);
                    $user = pdo_fetch("select * from ".tablename('user')." where uid=:id",array(':id'=>$orderInfo['user_id']));
                    $back_price = $user['balance']+($orderInfo['allprice']/100);
                    $back_point = $user['point']+($orderInfo['deduction_point']);
                    pdo_update('user',['balance'=>$back_price,'point'=>$back_point],['uid'=>$orderInfo['user_id']]);

                    pdo_insert('user_balance_record',array(
                        'order_num'=>m('common')->createNO('order', 'ordersn', 'SH'),
                        'user_id'=>$orderInfo['user_id'],
                        'money'=>($orderInfo['allprice']),
                        'all_money'=>($back_price*100),
                        'status'=>2,
                    ...
                    #如果有积分抵扣的情况下，退回积分
                    if($orderInfo['deduction_point']>0)
                    {
                        pdo_insert('user_balance_record',array(
                            'order_num'=>m('common')->createNO('order', 'ordersn', 'SH'),
                            'user_id'=>$orderInfo['user_id'],
                            'money'=>$orderInfo['deduction_point']*100,
                            'all_money'=>($user['point']+$orderInfo['deduction_point']*100),
                            'status'=>2,
                            'create_time'=>date('Y-m-d H:i:s'),
                            'pay_time'=>date('Y-m-d H:i:s'),
                            'action'=>'buy_refund',
                ...
                #拒绝退款
                pdo_update('order',['status'=>8,'drawback_refuse_reason'=>$refundcontent,'refund_fail_time'=>date('Y-m-d H:i:s')],['ren_order_id'=>$item['id']]);
