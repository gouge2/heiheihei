...
//            新用户积分提现默认为空
        ...
        if ($_W['ispost']) {
            ...
            #商户入驻暂停
            if($data['status'] == 2){
                pdo_update('goods',['is_show'=>'N'],['shop_id'=>$item['id']]);
            }
    ...
    function delete()
    {
        ...
        $itemdd = pdo_fetchall("SELECT * FROM " . tablename('ewei_shop_order') . " WHERE status in( 1,2,4 ) AND merchid=" . $id);

        if ($itemdd){
            show_json(0, '此商户存在未完成的订单，请在订单完成后再执行删除!');
        }
        ...
        foreach ($items as $item) {
            ...
            // 查询商户下的商品id
            $goodsID = pdo_fetchall("SELECT `goods_id` FROM " . tablename('goods') . " WHERE shop_id=" . $item['id']);
            if ($goodsID) {
                foreach ($goodsID as $k=>$v) {
                    // 删除商品被收藏记录
                    pdo_delete('goods_collect',array('goods_id'=>$v['goods_id']));

                    // 删除视频关联商品
                    pdo_delete('short_live_goods',array('goods_id'=>$v['goods_id'],'from'=>'self'));
                }
            }

            // 删除商户添加的商品
            pdo_delete('ewei_shop_goods',array('merchid'=>$item['id']));
            pdo_delete('goods',array('shop_id'=>$item['id']));

            // 删除商户下所有订单
            pdo_delete('ewei_shop_order',array('merchid'=>$item['id']));
            pdo_delete('order',array('shop_d'=>$item['id']));
