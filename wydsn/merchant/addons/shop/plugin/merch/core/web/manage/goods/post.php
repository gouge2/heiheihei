...
//    }
    ...
    $data['is_fx_goods'] =intval($_GPC['is_fx_goods'])==1?'Y':'N';
...
//    }
    ...
    if(!$goods_lailu)
    {
        pdo_insert('goods',[
            ...
            'is_fx_goods' => (IS_DISTRIBUTION == 'Y') ? (empty($_GPC['is_fx_goods'])?'N':'Y') : 'N',
            'fx_profit_money' => (IS_DISTRIBUTION == 'Y') ? (intval($_GPC['fx_profit_money']*100)) : 0,
            'is_show'=>$is_show,
            'brand_id'=>0,
            'ren_good_id'=>$id,
            'inventory'=>empty($data['total'])?0:$data['total'],
            'virtual_volume'=>empty($data['sales'])?0:$data['sales'],
            'postage' => empty($data['dispatchprice'])?0:trim($data['dispatchprice']),
        ]);
        #插入商品图信息
        $goods_id = pdo_insertid();

        foreach ($thumb_img as $it)
        {
