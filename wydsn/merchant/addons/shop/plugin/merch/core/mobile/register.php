...
class Register_EweiShopV2Page extends PluginMobileLoginPage 
{
	public function main() 
	{
		...
		$reg = pdo_fetch('select * from ' . tablename('ewei_shop_merch_reg') . ' where openid=:openid and uniacid=:uniacid limit 1', array(':openid' => $_GPC['openid'], ':uniacid' => $_W['uniacid']));
		$user = false;
		if (!(empty($reg['status']))) 
		{
		...
		#获取商户的入驻协议
        $article= pdo_fetch("select * from ".tablename('article')." where title like '%商户入驻%'");
		if($article)
        {
            $set['applytitle'] = $article['title'];
            $apply_set['applytitle']= $article['title'];
            $set['applycontent'] = $article['content'];
        }
		$template_flag = 0;
		...
		if ($_W['ispost']) 
		{
            ...
            $user = m('member')->getUserDetail($_GPC['openid']);
			if(!$user)
            {
                show_json(0, '入驻用户错误');
            }
			...
			$data = array('uniacid' => $_W['uniacid'], 'openid' => $_GPC['openid'], 'status' => 0, 'realname' => trim($_GPC['realname']), 'mobile' => trim($_GPC['mobile']), 'uname' => $uname, 'upass' => $upass, 'merchname' => $user['nickname'].'的店铺', 'address'=>trim($_GPC['address_detail']),'salecate' => trim($_GPC['salecate']),'cus_serve'=>trim($_GPC['cus_serve']), 'desc' => trim($_GPC['desc']));
			if ($template_flag == 1) 
			{
				$mdata = $_GPC['mdata'];
