...
class Index_EweiShopV2Page extends PluginMobilePage
{
    ...
    {
        global $_W;
        $data = pdo_fetchall("select * from ".tablename('goods_cat'));

        foreach ($data as $item)
        {
            $cat = pdo_fetch("select * from ".tablename('ewei_shop_category')." where name=:name and parentid=:parentid",array(":name"=>$item['cat_name'],":parentid"=>$item['parent_id']));
            if(!$cat)
            {
                pdo_insert('ewei_shop_category',['name'=>$item['cat_name'],'uniacid'=>$_W['uniacid'],'parentid'=>$item['parent_id'],'id'=>$item['cat_id'],'thumb'=>$item['img'],'enabled'=>($item['is_show']=='Y' ?'1':'0') ]);
            }
        }
        $this->message('操作成功');
    }
