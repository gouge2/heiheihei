...
class Index_EweiShopV2Page extends PluginWebPage
{
	...
	public function setting()
	{
		...
		if ($_W['ispost']) {
			...
			if (!empty($arr['keyword'])) {
				...
				$rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name  limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'shop', ':name' => 'shop:mmanage'));

				if (!empty($rule)) {
					pdo_update('rule_keyword', array('content' => $arr['keyword']), array('rid' => $rule['id']));
				}
				else {
					$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'ewei_shopv2:mmanage', 'module' => 'shop', 'displayorder' => 0, 'status' => 1);
					pdo_insert('rule', $rule_data);
					$rid = pdo_insertid();
					$keyword_data = array('uniacid' => $_W['uniacid'], 'rid' => $rid, 'module' => 'shop', 'content' => $arr['keyword'], 'type' => 1, 'displayorder' => 0, 'status' => 1);
	...
	protected function delKey($keyword)
	{
		...
		$keyword = pdo_fetch('SELECT * FROM ' . tablename('rule_keyword') . ' WHERE content=:content and module=:module and uniacid=:uniacid limit 1 ', array(':content' => $keyword, ':module' => 'shop', ':uniacid' => $_W['uniacid']));
