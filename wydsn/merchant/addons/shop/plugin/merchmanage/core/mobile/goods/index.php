...
class Index_EweiShopV2Page extends MerchmanageMobilePage
{
    ...
    protected function post()
    {
        ...
        $merch_user = pdo_fetch("select * from ".tablename('ewei_shop_merch_user')." where id=:id",array(":id"=>$_W['merchmanage']['merchid']));
        ...
        if ($_W['ispost']) {
            ...
            $data['is_fx_goods'] = trim($_GPC['is_fx_goods']);
            $data['fx_profit_money'] = trim($_GPC['fx_profit_money']);
            ...
            #获取已绑定的商品信息
            $goods_lailu = pdo_fetch("select * from ".tablename("goods")." where ren_good_id=:id",array(":id"=>$id));
            $is_show = !empty($data['status']) && empty($data['checked']) ? 'Y':'N';
            if(!empty($data['cates']))
            {
                $cat_goods = explode(',',$data['cates']);
            ...
            {
                pdo_insert('goods',[
                    'shop_id'=>$_W['merchmanage']['merchid'],
                    'goods_name'=>$data['title'],
                    'cat_id'=>$cat_goods,
                    'content'=>$data['content'],
                    'img'=>'/Public/Upload/Goods/images/'.$data['thumb'],
                    'tmp_img'=>'/Public/Upload/Goods/images/'.$data['thumb'],
                    'old_price'=>$data['productprice'],
                    'price'=>intval($data['marketprice']*100),
                    'virtual_volume'=>$data['sales'],
                    'is_fx_goods' => (IS_DISTRIBUTION == 'Y') ? (empty($data['is_fx_goods'])?'N':'Y') : 'N',
                    'fx_profit_money' => (IS_DISTRIBUTION == 'Y') ? (intval($data['fx_profit_money']*100)) : 0,
                    'is_show'=>$is_show,
                    'brand_id'=>0,
                    'ren_good_id'=>$id,
                    'inventory'=>empty($data['total'])?0:$data['total'],
                    'postage' => empty($_GPC['dispatchprice'])?0:trim($_GPC['dispatchprice']),
                ]);
                #插入商品图信息
                $is_ios=0;
                ...
                foreach ($_GPC['thumbs'] as $it)
                {
                    pdo_insert('goods_img',['goods_id'=>$goods_id,'title'=>'','img'=>'/Public/Upload/Goods/images/'.$it,'sort'=>0,'createtime'=>date('Y-m-d H:i:s')]);
                }
            }else{
                #如果只是更新的话，只更新价格以及上架的状态信息等
                #插入商品图信息
                $is_ios=0;
                if(strpos(strtolower($_SERVER['HTTP_USER_AGENT']),'iphone'))
                {
                ...
                #先删除原来的图片
                pdo_delete('goods_img',['goods_id'=>$goods_lailu['goods_id']]);
                foreach ($_GPC['thumbs'] as $it)
                {
                    pdo_insert('goods_img',['goods_id'=>$goods_lailu['goods_id'],'title'=>'','img'=>'/Public/Upload/Goods/images/'.$it,'sort'=>0,'createtime'=>date('Y-m-d H:i:s')]);
                }
            }
    ...
    public function delete()
    {
        ...
        foreach ($items as $item ) {
            ...
            #同步平台下架状态
            pdo_update('goods',['is_show'=>'N','is_delete'=>'Y'],['ren_good_id'=>$item['id']]);
    ...
    public function status()
    {
        ...
        foreach ($items as $item ) {
            ...
            #同步平台下架状态
            pdo_update('goods',['is_show'=>empty($_GPC['status'])?'N':'Y'],['ren_good_id'=>$item['id']]);
    ...
    public function restore()
    {
        ...
        foreach ($items as $item ) {
            ...
            #同步平台删除状态
            pdo_update('goods',['is_show'=>'N','is_delete'=>'N'],['ren_good_id'=>$item['id']]);
