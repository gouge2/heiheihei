...
class Common_EweiShopV2Model {
    ...
    public function getSetData($uniacid = 1) {
        global $_W;
        if (empty($uniacid)) {
            $uniacid = $_W['uniacid'];
        }
    ...
    public function getPluginset($key = '', $uniacid = 1) {
        global $_W, $_GPC;
        $set = $this->getSetData($uniacid);
    ...
    public function deleteSysset($key, $uniacid = 1) {
        global $_W, $_GPC;
        if (empty($uniacid)) {
            $uniacid = $_W['uniacid'];
        }
    ...
    public function updatePluginset($values, $uniacid = 1) {
        global $_W, $_GPC;
        if (empty($uniacid)) {
            $uniacid = $_W['uniacid'];
        }
    ...
    public function alipay_build($params, $alipay = array(), $type = 0, $openid = '') {
        ...
        if($pay['alipay_id']>0){
            ...
            if (empty($type)) {
                $config['notify_url'] = $_W['siteroot'] . "addons/shop/payment/alipay/notify.php";
            ...
            $set['sign_type'] = 'MD5';
            if (empty($type)) {
                $set['notify_url'] = $_W['siteroot'] . "addons/shop/payment/alipay/notify.php";
    ...
    function wechat_jspay($params,$wechat,$type=0)
    {
        ...
        $package['notify_url'] = $_W['siteroot'] . "addons/shop/payment/wechat/notify.php";
    ...
    function wechat_child_build($params, $wechat, $type = 0) {
        ...
        $package['notify_url'] = empty($params['notify_url']) ? $_W['siteroot'] . "addons/shop/payment/wechat/notify.php" : $params['notify_url'];
        $package['trade_type'] = 'JSAPI';
    ...
    public function sendredpack($params)
    {
        ...
        if (is_array($certs)) {
            ...
            $certfile = IA_ROOT . "/addons/shop/cert/" . random(128);
            file_put_contents($certfile, $certs['cert']);
            $keyfile = IA_ROOT . "/addons/shop/cert/" . random(128);
            ...
            if(!empty($certs['root']))
            {
                $rootfile = IA_ROOT . "/addons/shop/cert/" . random(128);
    ...
    public function getSec($uniacid = 1) {
        global $_W;
        if (empty($uniacid)) {
            $uniacid = $_W['uniacid'];
        }
    ...
    public function paylog($log = '') {
        ...
        if (!empty($openpaylog)) {
            $path = IA_ROOT . "/addons/shop/data/paylog/" . $_W['uniacid'] . "/" . date('Ymd');
            if (!is_dir($path)) {
                load()->func('file');
                @mkdirs($path, '0777');
            }
    ...
    public function getAreas()
    {
        ...
        if (!empty($new_area)) {
            $file = IA_ROOT . "/addons/shop/static/js/dist/area/AreaNew.xml";
        } else {
            $file = IA_ROOT . "/addons/shop/static/js/dist/area/Area.xml";
        }
