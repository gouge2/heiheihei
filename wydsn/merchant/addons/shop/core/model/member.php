...
//            return pdo_fetch("SELECT {$types} FROM " . tablename('ewei_shop_member') . " WHERE  (openid=:openid or openid_wa=:openid) and uniacid=:uniacid limit 1", array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
    ...
    public function getUserInfo($openid = '')
    {
        ...
        if (!empty($uid)) {
            $result = pdo_fetch("SELECT * FROM ".tablename('user')." WHERE uid=:uid",array(":uid"=>$uid));
            return $result;
        }
        return false;
    }
    ...
    public function getUserDetail($openid = '')
    {
        ...
        if (!empty($uid)) {
            $result = pdo_fetch("SELECT * FROM ".tablename('user_detail')." WHERE user_id=:uid",array(":uid"=>$uid));
            return $result;
        }
        return false;
    }
