...
class Set_EweiShopV2Page extends ComWebPage
{
	...
	'verify' => array('keyword' => $keyword, 'type' => $type)
			...
			$rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name  limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'shop', ':name' => 'shop:com:verify'));

			if (empty($rule)) {
				$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'ewei_shopv2:com:verify', 'module' => 'shop', 'displayorder' => 0, 'status' => 1);
				pdo_insert('rule', $rule_data);
				$rid = pdo_insertid();
				$keyword_data = array('uniacid' => $_W['uniacid'], 'rid' => $rid, 'module' => 'shop', 'content' => trim($keyword), 'type' => 1, 'displayorder' => 0, 'status' => 1);
