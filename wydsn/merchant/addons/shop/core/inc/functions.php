...
if (!function_exists('com_perm_getPermset')) {
    ...
    {
        $path = IA_ROOT . "/addons/shop/data/global";
    ...
    function com_perm_check_plugin($pluginname = ''){
        ...
        if (!empty($ac_perm)) {
            ...
            if (!in_array($pluginname, $allow_plugins)) {
                ...
                $filename = "../addons/shop/core/model/grant.php";
        ...
        if (is_array2($list)) {
            ...
            foreach ($list as $key => &$value) {
                ...
                foreach ($fields as $field) {
                    if (isset($list[$field])) {
                        $list[$field] = tomedia1($list[$field]);
                    }
                    if (is_array($value) && isset($value[$field])) {
                        $value[$field] = tomedia1($value[$field]);
                    }
                }
            }
            return $list;
        ...
        foreach ($options['items'] as $item) {
            if ($options['type'] == 'image') {
                $html .= "<div class='multi-item' data-{$options['key']}='{$item[$options['key']]}' data-name='{$name}'>
                                      <img class='img-responsive img-thumbnail' src='" . tomedia($item[$options['thumb']]) . "' onerror='this.src=\"../addons/shop/static/images/nopic.png\"' style='width:100px;height:100px;'>
                ...
                $html .= "
                    <tr class='multi-product-item' data-{$options['key']}='{$item['id']}' >
                       ...
                       <input type='hidden'  value='{$item['goodsid']}' name='{$id}'>
                        <td style='width:80px;'>
                            <img src='" . tomedia($item[$options['thumb']]) . "' style='width:70px;border:1px solid #ccc;padding:1px' onerror=\"this.src='../addons/shop/static/images/nopic.png'\">
                        </td>
...
if (!function_exists('mobileUrl')) {
    function mobileUrl($do = '', $query = null, $full = false)
    {
        ...
        $query = array_merge(array('m' => "shop"), $query);
...
if (!function_exists("tpl_form_field_image2")) {
    function tpl_form_field_image2($name, $value = '', $default = '', $options = array())
    {
        ...
        if (empty($default)) {
            $default = '../addons/shop/static/images/nopic.png';
        }
        $val = $default;
        if (!empty($value)) {
            $val = tomedia($value);
        } else {
            $val = '../addons/shop/static/images/default-pic.jpg';
        }
		...
		<script type="text/javascript">
			...
			function deleteImage(elm){
				require(["jquery"], function($){
					$(elm).prev().attr("src", "../addons/shop/static/images/default-pic.jpg");
	...
	function uploadMultiImage(elm) {
		...
		util.image( "", function(urls){
			$.each(urls, function(idx, url){
				$(elm).parent().parent().next().append(\'<div class="multi-item"><img onerror="this.src=\\\'../addons/shop/static/images/nopic.png\\\'; this.title=\\\'图片未找到.\\\'" src="\'+url.url+\'" class="img-responsive img-thumbnail"><input type="hidden" name="\'+name+\'[]" value="\'+url.attachment+\'"><em class="close" title="删除这张图片" onclick="deleteMultiImage(this)">×</em></div>\');
			});
		}, ' . json_encode($options) . ');
	}
...
<div class="multi-item">
	<img src="' . tomedia1($row) . '" onerror="this.src=\'../addons/shop/static/images/nopic.png\'; this.title=\'图片未找到.\'" class="img-responsive img-thumbnail">
...
if (!function_exists('tpl_goods_selector')) {
    ...
    function tpl_goods_selector($name, $data_gs = '', $option = array())
    {
        ...
        //包含模板文件
        $path = IA_ROOT . '/addons/shop/template/web_v3/util/tpl_goods_selector.html';
        include $path;
