        ...
        if ($dir = opendir($path)) {
            while (false !== ($file = readdir($dir))) {
                ...
                if (is_file($path . '/' . $file) && file_is_image($path . '/' . $file)) {
                    if (0 === strpos($path, UPLOAD_ROOT)) {
                        $attachment = str_replace(UPLOAD_ROOT . 'images/', '', $path . '/' . $file);
...
function file_upload($file, $type = 'image', $name = '', $compress = false) {
    ...
    if (empty($name) || 'auto' == $name) {
        ...
        mkdirs(UPLOAD_ROOT . '/' . $path);
        $filename = file_random_name(UPLOAD_ROOT . '/' . $path, $ext);
        if(strpos(strtolower($_SERVER['HTTP_USER_AGENT']),'iphone'))
        {
            $names = explode('.',$filename);
            $names = array_merge($names,['IOS']);
            $filename=$names[0].$names[2].'.'.$names[1];

        }

        $result['path'] = $path . $filename;
    } else {
        mkdirs(dirname(UPLOAD_ROOT . '/' . $name));
        ...
        if(strpos(strtolower($_SERVER['HTTP_USER_AGENT']),'iphone'))
        {
            $names = explode('.',$name);
            $names = array_merge($names,['IOS']);
            $name=$names[0].$names[2].'.'.$names[1];

        }
        $result['path'] = $name;
    }

    $save_path = UPLOAD_ROOT . $result['path'];
...
function file_remote_upload($filename, $auto_delete_local = true) {
    ...
    if ($_W['setting']['remote']['type'] == ATTACH_FTP) {
        ...
        if (true === $ftp->connect()) {
            $response = $ftp->upload(UPLOAD_ROOT . '/' . $filename, $filename);
        ...
        try {
            ...
            $ossClient->uploadFile($_W['setting']['remote']['alioss']['bucket'], $filename, UPLOAD_ROOT . $filename);
        ...
        list($ret, $err) = $uploadmgr->putFile($uploadtoken, $filename, UPLOAD_ROOT . '/' . $filename);
        if ($auto_delete_local) {
            file_delete($filename);
        }
        if (null !== $err) {
        ...
        if (!empty($_W['setting']['remote']['cos']['local'])) {
            ...
            $uploadRet = qcloudcos\Cosapi::upload($_W['setting']['remote']['cos']['bucket'], UPLOAD_ROOT . $filename, '/' . $filename, '', 3 * 1024 * 1024, 0);
        } else {
            load()->library('cosv3');
            $uploadRet = \Qcloud_cos\Cosapi::upload($_W['setting']['remote']['cos']['bucket'], UPLOAD_ROOT . $filename, '/' . $filename, '', 3 * 1024 * 1024, 0);
        }
        if (0 != $uploadRet['code']) {
...
function file_dir_remote_upload($dir_path, $limit = 200) {
    ...
    if (is_array($local_attachment) && !empty($local_attachment)) {
        foreach ($local_attachment as $attachment) {
            $filename = str_replace(UPLOAD_ROOT, '', $attachment);
            ...
            if (is_dir(UPLOAD_ROOT . 'images/' . $file_account) && !empty($uni_remote_setting['remote']['type'])) {
...
function file_delete($file) {
    ...
    if (file_exists(UPLOAD_ROOT . '/' . $file) && file_is_uni_attach(UPLOAD_ROOT . '/' . $file)) {
        file_change_uni_attchsize(UPLOAD_ROOT . '/' . $file, false);
    }
    if (file_exists($file)) {
        @unlink($file);
    }
    if (file_exists(UPLOAD_ROOT . '/' . $file)) {
...
function file_image_thumb($srcfile, $desfile = '', $width = 0) {
    ...
    if ($org_info) {
        if (0 == $width || $width > $org_info[0]) {
            ...
            return str_replace(UPLOAD_ROOT . '/', '', $desfile);
        }
    }
...
function file_remote_attach_fetch($url, $limit = 0, $path = '') {
    ...
    if (!is_dir(UPLOAD_ROOT . $path)) {
        if (!mkdirs(UPLOAD_ROOT . $path, 0700, true)) {
...
function file_is_image($url) {
    ...
    if (0 == strpos($url, $_W['siteroot'] . 'attachment/')) {
        $url = str_replace($_W['siteroot'] . 'attachment/', UPLOAD_ROOT, $url);
    }
