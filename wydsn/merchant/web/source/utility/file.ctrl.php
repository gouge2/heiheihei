...
if ('fetch' == $do) {
	...
	$filename = file_random_name(UPLOAD_ROOT . '/' . $setting['folder'], $ext);
...
if ('upload' == $do) {
	...
	$pathname = $file['path'];
	$fullname = UPLOAD_ROOT.$pathname;#UPLOAD_ROOT . '/' . $pathname;
}
		...
		foreach ($list as &$meterial) {
			if ($islocal) {
				if ($uniacid == 0) {
                    ...
                    $src=UPLOAD_ROOTS.$meterial['attachment'];
					$meterial['url'] =$_SERVER['REQUEST_SCHEME'].'://'.$_SERVER['SERVER_NAME'].'/Public/Upload/Goods/images/'.$meterial['attachment'];
//                        tomedia($meterial['attachment']);
				}
				unset($meterial['uid']);
			} else {
				if (!empty($_W['setting']['remote']['type'])) {
