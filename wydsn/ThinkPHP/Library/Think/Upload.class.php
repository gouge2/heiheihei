...
class Upload {
     ...
     * @param  string  $filename_type:文件名类型，1无随机数
     * @return array        上传成功后的文件信息
     */
    public function uploadOne($file,$filename_type=''){
        $info = $this->upload(array($file),$filename_type);
     ...
     */
    public function upload($files='',$filename_type='') {
        if('' === $files){
            $files  =   $_FILES;
        }
        if(empty($files)){
        ...
        foreach ($files as $key => $file) {
            ...
            /* 生成保存文件名 */
            $savename = $this->getSaveName($file,$filename_type);
            if(false == $savename){
                continue;
            } else {
     ...
     * @param  string  $filename_type:文件名类型，1无随机数
     */
    private function getSaveName($file,$filename_type='') {
        $rule = $this->saveName;
        ...
        // uniqid()函数可用于生成不重复的唯一标识符，该函数基于微秒级当前时间戳。在高并发或者间隔时长极短（如循环代码）的情况下，会出现大量重复数据。
        // 在uniqid()函数基础上有加上了3位随机数来确保高并发下文件名不重复
        if($filename_type==1){
            return $savename . '.' . $ext;
        }else {
            return $savename.rand(100, 999) . '.' . $ext;
        }
    }
