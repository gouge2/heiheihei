...
class traverseDir
{
    ...
    public function tozip($items)
    {
        ...
        $fileToZip=$this->savepath.$zipname.'.zip';
        if (!file_exists($zipname))
        {
        	// 创建一个空的zip文件
            if( ($zip->open($fileToZip,1)) ===true )
            {
    	...
    	while ( ($f = readdir($handle))!==false )
    	{
    		if ($f != "." && $f != "..") 
    		{
    			if (is_file($folder . $f)) 
    			{
    				...
    				// 如果是文件夹，创建一个子文件夹在ZIP压缩包中
    				$zipFile->addEmptyDir($folder . $f);
    				// 递归调用
    				$this->folderToZip($folder . $f, $zipFile, $folder . $f);
    			}
    		}
    	}
    	return true;
