...
//		$input->SetTime_expire(date("YmdHis", time() + 600));
     ...
     * @param String(32) $out_trade_no:必填，商户系统内部订单号，要求32个字符内、且在同一个商户号下唯一
     * @param int $total_fee:必填，订单总金额，单位为分
     * * @param int $openid:必填，下单用户的openid
     * @param String(255) $notify_url:必填，异步接收微信支付结果通知的回调地址，通知url必须为外网可访问的url，不能携带参数
     * @param string $attach
     * @param String(32) $goods_tag:非必填，商品标记，使用代金券或立减优惠功能时需要的参数
     * @return array:支付表单数据
     */
    public function GetMiniAppParameters($body,$out_trade_no,$total_fee,$openid, $notify_url,$attach='',$goods_tag='')
    {
        //①请求prepay_id
        //统一下单
        $input = new WxPayUnifiedOrder();
...
//        $input->SetTime_expire(date("YmdHis", time() + 600));
        ...
        $order = WxPayApi::unifiedOrderMin($input);
//        #如果是创建失败的情况下
...
//        }
        ...
        $AppParameters = $tools->GetAppParametersMini($order);
        return $AppParameters;
    }
	...
	public function SpBill(){
	    $input = new WxPaySpbill();
	    $input->SetAmount(100);
	    $input->SetCheck_name("NO_CHECK");
	    $input->SetOpenid("1234");
	    $input->SetDesc("提现");
	    WxPayApi::spbill($input);
    }
}