...
class pdd
{
	...
	public function searchGoods($keyword='',$opt_id='',$page=1,$page_size=100,$sort_type,$with_coupon,$range_list='',$cat_id='',$goods_id_list='',$zs_duo_id='',$merchant_type='')
	{
		...
		$req=array(
				...
				'pid'=> PDD_PID,
				'custom_parameters'=> json_encode(['new' => 1]),
		);
		$resp = $c->execute($req);
		$result=json_decode($resp,true);
	 ...
	 * @return mixed
	 */
	public function goodsPromotionUrlGenerate($p_id,$goods_id_list,$generate_short_url='',$multi_group='false',$custom_parameters='',$pull_new='',$generate_weapp_webview='false',$zs_duo_id='',$generate_we_app='', $generate_coupon = '')
	{
		$c = new PddClient();
		...
		$req=array(
				...
				'generate_mall_collect_coupon'=>$generate_coupon,
		);
		$resp = $c->execute($req);
		$result=json_decode($resp,true);
		if($result['error_response']['error_code'])
	 ...
	 * 多多进宝转链接口
	 */
	public function getUnitUrl($source_url, $uid)
	{
		$c 			= new PddClient();
		$c->type	= 'pdd.ddk.goods.zs.unit.url.gen';
		$req 		= [
			'pid' 				=> PDD_PID,
	 ...
	 * 生成多多进宝频道推广
	 */
	public function getResourceUrl($url, $uid)
	{
		$c 			= new PddClient();
		$c->type	= 'pdd.ddk.resource.url.gen';
		$req 		= [
			'pid' 				=> PDD_PID,
			'url'        		=> $url,
			'custom_parameters' => $uid,
	 ...
	 * 处理成简洁的数据列表
	 */
	public function pddConciseList($pdd_gid, $fee_user, $group_id, $res_tag = false)
	{
		$s_pdd   	= S('pdd_data'); 
		$pdd_data   = $s_pdd ? $s_pdd : [];    // 读取缓存有没有相应的数据
		$new_data   = [];
		$pdd_sw     = false;

        // 主播佣金
        $HostTreatModel = new \Common\Model\HostTreatModel();
        $hostDetail = $HostTreatModel->getHostCommission($pdd_gid, 'pdd', $group_id, '');
        $hostCommission = $hostDetail['commission'];

		if ($pdd_gid) {
			foreach ($pdd_gid as $key => $val) {
				if (!isset($pdd_data[$val])) {      // 未缓存拼多多商品信息  去获取
					$res        = $this->getGoodsDetail("[{$val}]");

					if ($res && isset($res['code']) && $res['code'] == 0) {
						// 价格计算
						$tem_price      = $res['data']['goods_details']['min_group_price']*1 - $res['data']['goods_details']['coupon_discount']*1;
						$price          = substr(sprintf("%.3f", $tem_price / 100), 0, -1);
						$old_price      = substr(sprintf("%.3f", $res['data']['goods_details']['min_group_price'] / 100), 0, -1);
						// 佣金比例
						$commission     = ($tem_price * $res['data']['goods_details']['promotion_rate'] / 1000)  / 100;
						// 优惠券
						$coupon_amount  = substr(sprintf("%.1f", $res['data']['goods_details']['coupon_discount'] / 100), 0, -2); 

						$pdd_data[$val] = $new_data[]   = [
							'goods_id'        => $val,
							'goods_url'       => '',
							'goods_name'      => $res['data']['goods_details']['goods_name'],
							'img'             => $res['data']['goods_details']['goods_image_url'],
							'price'           => $price,
							'old_price'       => $old_price,
							'sales_volume'    => (string)$res['data']['goods_details']['sales_tip'],
							'from'            => 'pdd',
							'commission_base' => $commission,
							'commission'      => sprintf("%.2f", ($commission * $fee_user / 100)),
							'coupon_amount'   => $coupon_amount,
                            'commission_host'      => sprintf("%.2f", ($commission * $hostCommission)),
						];

                        // 如果已经添加到商品夹，走主播的分佣
                        if ($hostCommission['userCommission'] && in_array($val, $hostCommission['goodsList'])) {
                            $pdd_data[$val]['commission'] = sprintf("%.2f", ($commission * $hostCommission['userCommission']));
                        }
					...
					// 佣金转换
					$pdd_data[$val]['commission'] = sprintf("%.2f", ($pdd_data[$val]['commission_base'] * $fee_user / 100));
					$pdd_data[$val]['commission_host'] = sprintf("%.2f", ($pdd_data[$val]['commission_base'] * $hostCommission));

                    // 如果已经添加到商品夹，走主播的分佣
                    if ($hostCommission['userCommission'] && in_array($val, $hostCommission['goodsList'])) {
                        $pdd_data[$val]['commission'] = sprintf("%.2f", ($pdd_data[$val]['commission_base'] * $hostCommission['userCommission']));
                    }

					if ($res_tag) {
						$new_data[] = $pdd_data[$val];
					}
				}
			}
		}
		...
		if ($pdd_sw) {
			S('pdd_data', $pdd_data, 86400);  // 保存缓存1天  避免重复调用接口
		}

		return $res_tag ? $new_data : $pdd_data;
	}
    ...
    {
        $c = new PddClient();
        $c->type = 'pdd.ddk.member.authority.query';
        $req = [
            'pid' => PDD_PID,
            'custom_parameters' => $uid,
        ];
        ...
        if (isset($result['error_response'])) {
            $this->ajaxError($result['error_response']['error_code'],$result['error_response']['error_msg']);
        }
     ...
     * @return mixed
     */
    public function GenerateAuthLinkApplets($uid)
    {
        $c = new PddClient();
        $c->type ='pdd.ddk.rp.prom.url.generate';
        $req = [
            'channel_type' => 10,
            'generate_we_app' => 'true',
            'p_id_list' => '["'.PDD_PID.'"]',
            'custom_parameters' => json_encode(['uid'=>$uid]),
        ];
        $resp = $c->execute($req);
        $result = json_decode($resp, true);
