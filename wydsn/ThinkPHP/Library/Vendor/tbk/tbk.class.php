...
class tbk
{
	...
	protected $url_dtk='https://openapi.dataoke.com/api';//接口地址-大淘客
	 ...
	 * @param string $relationId:淘宝渠道关系ID
	 * @return array
	 * @return @param code:返回码
	 * @return @param msg:返回码说明
	 * @return @param data:返回数据
	 ...
	 * @return @param data->commission:佣金
	 */
	public function getItemDetail($num_iid,$platform='1',$ip='',$pid='',$relationId ='',$version = '')
	{
		if($pid=='') {
			//没有的情况下使用默认推广位
			$pid=$this->pid;
		...
		$res_commsion = $this->getGoodsDetail('',$num_iid);
		if($res_info['code']==0) {
			//获取卖点信息
            ...
            if ($version) {
                $res_info['data']['zk_final_price']=substr(sprintf("%.3f",$res_commsion['data']['originalPrice']),0,-1);
            }
            ...
            //调用大淘客高效转链接口
            $result_gy = $this->getPrivilegeLink($num_iid, $pid, $relationId);
            $gy_data=$result_gy['data'];

			//生成渠道分享链接
            if($relationId){
                $res_info['data']['coupon_click_url_r']=$gy_data['couponClickUrl'];
            }else{
			...
			//优惠券总量
			if($gy_data['couponTotalCount']) {
				$res_info['data']['coupon_total_count']=$gy_data['couponTotalCount'];
			}else {
				$res_info['data']['coupon_total_count']=0;
			}
			//优惠券剩余量
			if($gy_data['couponRemainCount']) {
				$res_info['data']['coupon_remain_count']=$gy_data['couponRemainCount'];
			}else {
				$res_info['data']['coupon_remain_count']=0;
			}
			//优惠券开始时间
			if($gy_data['couponStartTime']) {
				$res_info['data']['coupon_start_time']=$gy_data['couponStartTime'];
			}else {
				$res_info['data']['coupon_start_time']='';
			}
			//优惠券结束时间
			if($gy_data['couponEndTime']) {
				$res_info['data']['coupon_end_time']=$gy_data['couponEndTime'];
			}else {
				$res_info['data']['coupon_end_time']='';
			}
			//优惠券信息
			if($gy_data['couponInfo']) {
				$res_info['data']['coupon_info']=$gy_data['couponInfo'];
				//优惠券面额
				$pos1=strpos($gy_data['couponInfo'],'减');
				$pos2=strripos($gy_data['couponInfo'],'元');
				$res_info['data']['coupon_amount']=substr($gy_data['couponInfo'], $pos1+3,$pos2-$pos1-3);
			}else {
			...
			if($gy_data['couponClickUrl']) {
				$res_info['data']['coupon_click_url']=$gy_data['couponClickUrl'];
			}else {
				$res_info['data']['coupon_click_url']="https://uland.taobao.com/coupon/edetail?itemId=$num_iid&pid=$pid";
			}
			//佣金比率(%)
			//维易淘客
			$res_info['data']['commission_rate']=$gy_data['maxCommissionRate'];
			//上海淘客
			//$res_info['data']['commission_rate']=$gy_data['max_commission_rate'];
			//佣金
			$res_info['data']['commission']=$res_commsion['data']['actualPrice']*$res_commsion['data']['commissionRate']/100;
			//保留2位小数，四舍五不入
			$res_info['data']['commission']=substr(sprintf("%.3f",$res_info['data']['commission']),0,-1);

			//商品详情页面地址
			$res_info['data']['item_url']=$gy_data['itemUrl'];
     ...
     * @return array
     */
    public function getPrivilegeLink($goodsId, $pid='', $relationId='') {
        ...
        $str = trim($str, '&');
        $data['sign'] = strtoupper(md5($str . '&key=' . $this->dtksecret));

        //拼接请求地址
        $url = $this->url_dtk .'/tb-service/get-privilege-link'. '?' . http_build_query($data);
        //执行请求获取数据
        $ch = curl_init();
    ...
    public function getGoodsDetail($id,$goodsId) {
        ...
        //拼接请求地址
        $url = $this->url_dtk .'/goods/get-goods-details'. '?' . http_build_query($data);
        //执行请求获取数据
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $url);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
	...
	public function getTqgJu($pid,$adzone_id,$start_time,$end_time,$page_no=1,$page_size=40,$fee_user=100)
	{
		...
		if($result['code']) {
			...
			for($i=0;$i<$num;$i++) {
				...
				//优惠券总量
				if($gy_data['couponTotalCount']) {
					$goodslist[$i]['coupon_total_count']=$gy_data['couponTotalCount'];
				}else {
					$goodslist[$i]['coupon_total_count']=0;
				}
				//优惠券剩余量
				if($gy_data['couponRemainCount']) {
					$goodslist[$i]['coupon_remain_count']=$gy_data['couponRemainCount'];
				}else {
					$goodslist[$i]['coupon_remain_count']=0;
				}
				//优惠券开始时间
				if($gy_data['couponStartTime']) {
					$goodslist[$i]['coupon_start_time']=$gy_data['couponStartTime'];
				}else {
					$goodslist[$i]['coupon_start_time']='';
				}
				//优惠券结束时间
				if($gy_data['couponEndTime']) {
					$goodslist[$i]['coupon_end_time']=$gy_data['couponEndTime'];
				}else {
					$goodslist[$i]['coupon_end_time']='';
				}
				//优惠券信息
				if($gy_data['couponInfo']) {
					$goodslist[$i]['coupon_info']=$gy_data['couponInfo'];
					//优惠券面额
					$pos1=strpos($gy_data['couponInfo'],'减');
					$pos2=strripos($gy_data['couponInfo'],'元');
					$goodslist[$i]['coupon_amount']=substr($gy_data['couponInfo'], $pos1+3,$pos2-$pos1-3);
				}else {
					$goodslist[$i]['coupon_info']='';
					$goodslist[$i]['coupon_amount']=0;
				}
				//佣金比率(%)
				$goodslist[$i]['commission_rate']=$gy_data['maxCommissionRate'];
				//佣金
