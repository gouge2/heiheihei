...
class vip
{
    ...
    // todo 工具商权限需要用户授权完的token，渠道商不需要
    protected $access_token=VIP_ACCESS_TOKEN;
    ...
    public function getGoodsList($channelType=0,$page=1,$pageSize=20,$fieldName='COMMISSION',$order=0)
    {
        ...
        $ctx->setAppURL("https://gw.vipapis.com/");
        $ctx->setLanguage("zh");
        // todo 使用工具商接口，渠道商接口则禁掉下一行
        if ($this->access_token=='VIP_ACCESS_TOKEN' or $this->access_token==''){
            $res=array(
                'code'=>2,
                'msg'=>'唯品会未授权，暂时不能使用',
                'data'=>array()
        ...
        // todo 使用工具商接口，渠道商接口$service->goodsList
        $resp=$service->goodsListWithOauth($request1);
//        $resp=$service->goodsList($request1);
        $result=json_decode(json_encode($resp),true);
        if(!$result)
        {
            $res=array(
    ...
    public function getKeywordGoods($keyword,$page,$page_size,$field_name,$order,$price_start,$price_end)
    {
        ...
        // todo 使用工具商接口，渠道商接口$service->query
        $resp=$service->queryWithOauth($request1);
//        $resp=$service->query($request1);
        $result=json_decode(json_encode($resp),true);
        if(empty($result['goodsInfoList']))
        {
            $res=array(
                'code'=>1,
                'msg'=>'关键词查询失败',
...
//        $goodsIdList1[0]=$goods_id;
        // todo 使用工具商接口，渠道商接口$service->getByGoodsIds
        $resp = $service->getByGoodsIdsWithOauth($goods_ids,"requestId");
//        $resp = $service->getByGoodsIds($goods_ids,"requestId");
        $result=json_decode(json_encode($resp),true);
        if(empty($result))
        {
            $res=array(
    ...
    public function goodsByGoodsIdGetUrl($goods_id,$user_tag)
    {
        ...
        $goodsIdList1=array();
        $goodsIdList1[0]=$goods_id;
        // todo 使用工具商接口，渠道商接口$service->genByGoodsId
        $resp=$service->genByGoodsIdWithOauth($goodsIdList1,$user_tag,"requestId");
//        $resp=$service->genByGoodsId($goodsIdList1,$user_tag,"requestId");
        $result=json_decode(json_encode($resp),true);
        if(empty($result))
        {
            $res=array(
...
//        }
        ...
        // todo 使用工具商接口，渠道商接口$service->orderList
        $resp=$service->orderListWithOauth($queryModel1);
//        $resp=$service->orderList($queryModel1);
        $result=json_decode(json_encode($resp),true);
        if(empty($result))
        {
            $res=array(
